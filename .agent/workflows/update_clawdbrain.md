---
description: Update Clawdbrain from upstream when branch has diverged (ahead/behind)
---

# Clawdbrain Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Clawdbrain" || true

# Move old version
mv /Applications/Clawdbrain.app /tmp/Clawdbrain-backup.app

# Install new build
cp -R dist/Clawdbrain.app /Applications/

# Launch
open /Applications/Clawdbrain.app
```
...
pnpm clawdbrain health

# Verify no zombie processes
ps aux | grep -E "(clawdbrain|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm clawdbrain agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update clawdbrain.json with fallback chains
...
pnpm clawdbrain agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
