...
const BillingSubscription: React.FC<{ isLoading?: boolean }> = ({ isLoading = false }) => {
  ...
  const renderTabs = () => (
    <div role="tablist" className="flex space-x-1 border-b border-zinc-800 mb-8">
          ...
          <div>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-zinc-400">Your current billing cycle is {billingCycle}.</p>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-white">$49<span className="text-sm font-normal text-zinc-500">/mo</span></div>
                <div className="text-xs text-indigo-400 font-medium">Next renewal: June 1, 2024</div>
              </div>
            </div>
            <div className="flex items-center space-x-3 mb-6 bg-zinc-950/50 p-2 rounded-lg w-fit">
              <span className={cn("text-xs font-medium px-2 py-1 rounded", billingCycle === 'monthly' ? 'bg-indigo-500 text-white' : 'text-zinc-400 cursor-pointer')} onClick={() => setBillingCycle('monthly')}>Monthly</span>
              <span className={cn("text-xs font-medium px-2 py-1 rounded", billingCycle === 'annual' ? 'bg-indigo-500 text-white' : 'text-zinc-400 cursor-pointer')} onClick={() => setBillingCycle('annual')}>Annual (Save 20%)</span>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 border-t border-zinc-800 pt-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-zinc-300">Unlimited Agents</span>
            </div>
            <div className="flex items-center space-x-2">
              <svg className="w-5 h-5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
              <span className="text-sm text-zinc-300">10M Monthly Tokens</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              <div className="bg-zinc-800 px-2 py-1 rounded text-[10px] font-bold text-zinc-300">VISA</div>
              <div>
          ...
          <button className="w-full py-2 bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-medium rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none">
            Update Card
          </button>
          <div className="mt-6 pt-6 border-t border-zinc-800">
            <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Billing Address</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <div 
              key={plan.name} 
              className={cn(
                "bg-zinc-900 border rounded-xl p-6 flex flex-col",
                plan.current ? "border-indigo-500 ring-1 ring-indigo-500" : "border-zinc-800"
              )}
            >
              <div className="mb-6">
                <div className="flex justify-between items-center mb-1">
                  <h4 className="text-lg font-bold text-white">{plan.name}</h4>
                  {plan.current && <span className="bg-indigo-500 text-[10px] font-bold text-white px-2 py-0.5 rounded-full uppercase">Current</span>}
                </div>
                <p className="text-sm text-zinc-500 mb-4">{plan.description}</p>
                <div className="text-3xl font-bold text-white">
                  {plan.price}
                  {plan.price !== "Custom" && <span className="text-sm font-normal text-zinc-500">/mo</span>}
                </div>
              </div>
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-300">
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-white mb-1">Usage Statistics</h2>
            <p className="text-zinc-400 text-sm">Current period: May 1, 2024 - May 31, 2024</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded-full text-xs font-medium text-amber-400">
            9 days remaining
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              return (
                <div key={meter.label}>
                  <div className="flex justify-between items-end mb-2">
                    <span className="text-sm font-medium text-white">{meter.label}</span>
                    <span className="text-xs text-zinc-400">
                      <span className="text-white font-bold">{meter.used}{meter.unit}</span> / {meter.limit}{meter.unit}
                    </span>
                  </div>
                  ...
                  <div className="mt-1.5 text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <h3 className="text-sm font-bold text-white mb-6 uppercase tracking-widest text-zinc-500">Token Consumption (Last 7 Days)</h3>
            <div className="h-48 flex items-end justify-between px-2">
              {[65, 42, 88, 95, 76, 54, 82].map((val, i) => (
                <div key={i} className="flex flex-col items-center group">
                  <div className="text-[10px] text-zinc-600 font-bold mb-2 opacity-0 group-hover:opacity-100 transition-opacity">{(val * 100).toLocaleString()}</div>
                  <div 
                  ...
                  <span className="text-[10px] text-zinc-500 mt-2 font-medium">05/{20+i}</span>
                </div>
              ))}
            </div>
            <div className="mt-8 grid grid-cols-2 gap-4">
              <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                <div className="text-[10px] text-zinc-500 font-bold uppercase mb-1">Peak Day</div>
                <div className="text-lg font-mono text-white">952k</div>
              </div>
              <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                <div className="text-[10px] text-zinc-500 font-bold uppercase mb-1">Avg / Day</div>
                <div className="text-lg font-mono text-white">712k</div>
              </div>
            </div>
          </div>
        </div>
  ...
  const renderInvoicesTab = () => (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Billing History</h2>
          <p className="text-zinc-400 text-sm">View and download your past invoices.</p>
        </div>
        <button className="flex items-center space-x-2 px-4 py-2 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-white text-sm font-medium rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none">
          ...
          <tbody className="divide-y divide-zinc-800">
            {invoices.map((invoice) => (
              <tr key={invoice.id} className="hover:bg-zinc-800/30 transition-colors">
                <td className="px-6 py-4 text-sm font-mono text-white">{invoice.id}</td>
                <td className="px-6 py-4 text-sm text-zinc-400">{invoice.period}</td>
                ...
                <td className="px-6 py-4 text-right">
                  {invoice.status === "Failed" ? (
                    ...
                    <button className="text-xs font-bold text-zinc-500 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none p-1 rounded">
                      Download PDF
                    </button>
                  )}
                </td>
    ...
    <div className="min-h-screen bg-zinc-950 p-6 md:p-12 text-white">
      <div className="max-w-6xl mx-auto">
        <header className="mb-10">
          ...
          <p className="text-zinc-500">Manage your plan, monitor usage, and view billing history.</p>
        </header>
