  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ── Header ──────────────────────────────────────────────────────────── */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Capacity Forecast Dashboard</h1>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 lg:grid-cols-4">
              {resourceCards.map(card => (
                <div key={card.type} className="bg-zinc-900 rounded-lg p-4 border border-zinc-700">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-zinc-400 text-sm font-medium">{card.type}</span>
                  ...
                  <div className="text-xs text-zinc-400">
                    <span className="text-zinc-300 font-medium">{card.used}</span>
            ...
            <div className="bg-zinc-900 rounded-lg border border-zinc-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-zinc-700 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-white">Service Utilization</h2>
                <span className="text-xs text-zinc-500">Click a row to expand hourly history</span>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-700 bg-zinc-800/40">
                      ...
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">CPU %</th>
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">Memory %</th>
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">Instances</th>
                      <th className="text-center px-4 py-3 text-zinc-400 font-medium">Auto-Scale</th>
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">Threshold</th>
                    </tr>
                  </thead>
                  <tbody>
                    {services.map(svc => (
                      <React.Fragment key={svc.id}>
                        <tr
                          ...
                          className="border-b border-zinc-700/50 hover:bg-zinc-800/50 cursor-pointer transition-colors"
                        >
                          <td className="px-4 py-3 font-mono text-zinc-300">
                            <span className="mr-2 text-zinc-500 text-xs select-none">
                          ...
                          <td className="px-4 py-3 text-right text-zinc-300">{svc.instances}</td>
                          ...
                          <td className="px-4 py-3 text-right text-zinc-400">{svc.threshold}%</td>
                        </tr>

                        {/* Expanded: 12h hourly bars */}
                        {expandedService === svc.id && (
                          <tr className="border-b border-zinc-700/50">
                            <td colSpan={6} className="px-4 py-4 bg-zinc-800/20">
                              ...
                              <div className="flex items-end gap-1.5" style={{ height: "72px" }}>
                                {svc.history.map((val, idx) => (
                                  <div key={idx} className="flex flex-col items-center gap-1 flex-1">
                                    ...
                                    />
                                    <span className="text-zinc-600" style={{ fontSize: "9px" }}>
                                      {idx === 11 ? "now" : `${11 - idx}h`}
                                    </span>
                                  </div>
                                ))}
        ...
        {activeTab === "forecast" && (
          <div>
            ...
            <div className="bg-zinc-900 rounded-lg border border-zinc-700 p-6 mb-6">
                ...
                {activeForecast.months.map(month => (
                  <div key={month.month}>
                    ...
                    <div className="flex items-center gap-3 mb-1.5">
                      <span className="text-xs text-zinc-500 w-16 text-right shrink-0">Current</span>
                      <div className="flex-1 h-5 bg-zinc-800 rounded overflow-hidden">
                        <div
                          className="h-full bg-indigo-500/60 rounded flex items-center px-2"
                          style={{ width: `${month.current}%` }}
                        >
                          <span className="text-xs text-white/90 whitespace-nowrap">{month.current}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Projected bar */}
                    <div className="flex items-center gap-3 mb-1.5">
                      <span className="text-xs text-zinc-500 w-16 text-right shrink-0">Projected</span>
                      <div className="flex-1 h-5 bg-zinc-800 rounded overflow-hidden">
                        <div
                        ...
                        >
                          <span className="text-xs text-white/90 whitespace-nowrap">{month.projected}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-zinc-500 w-16 text-right shrink-0">Headroom</span>
                      <div className="flex-1 h-5 bg-zinc-800 rounded overflow-hidden">
                        <div
              ...
              {/* Legend */}
              <div className="flex gap-5 mt-6 pt-5 border-t border-zinc-700">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-sm bg-indigo-500/60" />
                  <span className="text-xs text-zinc-400">Current baseline</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-sm bg-amber-400 opacity-80" />
                  <span className="text-xs text-zinc-400">Projected demand</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-sm bg-emerald-400/25 border border-emerald-400/20" />
                  <span className="text-xs text-zinc-400">Available headroom</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 rounded-lg border border-zinc-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-zinc-700">
                <h2 className="text-sm font-semibold text-white">Forecast Assumptions</h2>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-700 bg-zinc-800/40">
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Resource</th>
                    <th className="text-right px-4 py-3 text-zinc-400 font-medium">Growth Rate</th>
                    <th className="text-right px-4 py-3 text-zinc-400 font-medium">Confidence</th>
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Model</th>
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Last Updated</th>
                  </tr>
                </thead>
                <tbody>
                  {forecastData.map(fd => (
                    <tr key={fd.resource} className="border-b border-zinc-700/50 hover:bg-zinc-800/30">
                      <td className="px-4 py-3 font-medium text-zinc-200">{fd.resource}</td>
                      <td className="px-4 py-3 text-right text-amber-400 font-medium">{fd.growthRate}% / mo</td>
                      <td className="px-4 py-3 text-right text-zinc-400">{fd.confidenceInterval}</td>
                      <td className="px-4 py-3 text-zinc-300">{fd.model}</td>
                      <td className="px-4 py-3 font-mono text-xs text-zinc-400">{fd.lastUpdated}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === "planning" && (
          <div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-zinc-900 rounded-lg p-4 border border-amber-400/20">
                <div className="text-zinc-400 text-xs mb-1">⏳ Pending Approvals</div>
              ...
              <div className="bg-zinc-900 rounded-lg p-4 border border-indigo-500/20">
                <div className="text-zinc-400 text-xs mb-1">📅 Scheduled Changes</div>
              ...
              <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-700">
                <div className="text-zinc-400 text-xs mb-1">💰 Est. Monthly Cost Delta</div>
                ...
                <div className="text-xs text-zinc-500 mt-1">pending + scheduled</div>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 rounded-lg border border-zinc-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-zinc-700">
                <h2 className="text-sm font-semibold text-white">Capacity Change Requests</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-700 bg-zinc-800/40">
                      ...
                      <th className="text-left px-4 py-3 text-zinc-400 font-medium">Quantity</th>
                      <th className="text-left px-4 py-3 text-zinc-400 font-medium">Requester</th>
                      <th className="text-left px-4 py-3 text-zinc-400 font-medium">Target Date</th>
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">Cost / mo</th>
                  ...
                  <tbody>
                    {changeRequests.map(cr => {
                      const status = changeStatuses[cr.id]
                      return (
                        <tr key={cr.id} className="border-b border-zinc-700/50 hover:bg-zinc-800/30 transition-colors">
                          <td className="px-4 py-3 font-medium text-zinc-200">{cr.resourceType}</td>
                          <td className="px-4 py-3 text-zinc-400">{actionLabel(cr.action)}</td>
                          <td className="px-4 py-3 font-mono text-zinc-300">{cr.quantity}</td>
                          <td className="px-4 py-3 text-zinc-400">{cr.requester}</td>
                          <td className="px-4 py-3 font-mono text-xs text-zinc-400">{cr.targetDate}</td>
                          <td className={cn(
        ...
        {activeTab === "alerts" && (
          <div>
            {/* Severity Summary */}
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-zinc-900 rounded-lg p-4 border border-rose-400/25">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm">🔴</span>
                  <span className="text-zinc-400 text-xs font-medium">Critical</span>
                </div>
                <div className="text-3xl font-bold text-rose-400">{criticalCount}</div>
                <div className="text-xs text-zinc-500 mt-1">unacknowledged</div>
              </div>
              <div className="bg-zinc-900 rounded-lg p-4 border border-amber-400/25">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm">🟡</span>
                  <span className="text-zinc-400 text-xs font-medium">Warning</span>
                </div>
                <div className="text-3xl font-bold text-amber-400">{warningCount}</div>
                <div className="text-xs text-zinc-500 mt-1">unacknowledged</div>
              </div>
              <div className="bg-zinc-900 rounded-lg p-4 border border-indigo-500/25">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm">🔵</span>
                  <span className="text-zinc-400 text-xs font-medium">Info</span>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-700 bg-zinc-800/40">
                      ...
                      <th className="text-right px-4 py-3 text-zinc-400 font-medium">Threshold</th>
                      <th className="text-center px-4 py-3 text-zinc-400 font-medium">Severity</th>
                      <th className="text-left px-4 py-3 text-zinc-400 font-medium">Triggered At</th>
                      <th className="text-center px-4 py-3 text-zinc-400 font-medium">Ack&apos;d</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredAlerts.map(alert => (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-zinc-200">{alert.resource}</td>
                        <td className="px-4 py-3 font-mono text-zinc-400">{alert.service}</td>
                        <td className={cn("px-4 py-3 text-right font-semibold", severityTextColor(alert.severity))}>
                          {alert.currentValue}%
                        </td>
                        <td className="px-4 py-3 text-right text-zinc-400">{alert.threshold}%</td>
                        <td className="px-4 py-3 text-center">
                          <span className={cn("px-2 py-0.5 rounded text-xs font-medium", severityBadgeClass(alert.severity))}>
                            {alert.severity}
                          </span>
                        </td>
                        <td className="px-4 py-3 font-mono text-xs text-zinc-400">{alert.triggeredAt}</td>
                        <td className="px-4 py-3 text-center">
                          {alert.acknowledged ? (
