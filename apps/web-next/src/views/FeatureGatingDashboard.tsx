...
function statusBadge(s: GateStatus) {
  ...
  if (s === "disabled") {return "bg-[var(--color-surface-3)] text-[var(--color-text-secondary)]";}
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface-0)] text-[var(--color-text-primary)] p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Feature Gating Dashboard</h1>
          <p className="text-[var(--color-text-secondary)] text-sm mt-1">Manage feature flags, rollouts, and user targeting across environments</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
          <div className="text-xs text-[var(--color-text-muted)] mb-1">Enabled Gates</div>
          <div className="text-2xl font-bold text-emerald-400">{enabledCount}</div>
        </div>
        <div className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
          <div className="text-xs text-[var(--color-text-muted)] mb-1">Partial Rollout</div>
          <div className="text-2xl font-bold text-amber-400">{partialCount}</div>
        </div>
        <div className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
          <div className="text-xs text-[var(--color-text-muted)] mb-1">Disabled</div>
          <div className="text-2xl font-bold text-[var(--color-text-secondary)]">{disabledCount}</div>
        </div>
        <div className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
          <div className="text-xs text-[var(--color-text-muted)] mb-1">Total Evaluations</div>
          <div className="text-2xl font-bold text-[var(--color-text-primary)]">{(totalEvaluations / 1000000).toFixed(1)}M</div>
        </div>
      </div>
      ...
      {tab === "gates" && !selectedGate && (
        <div>
          <div className="flex flex-wrap gap-3 mb-4">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as GateStatus | "all")} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-1.5 text-sm text-[var(--color-text-primary)]">
              <option value="all">All Status</option>
              <option value="enabled">Enabled</option>
              <option value="partial">Partial</option>
              <option value="disabled">Disabled</option>
            </select>
            <select value={filterEnv} onChange={e => setFilterEnv(e.target.value as Environment | "all")} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-1.5 text-sm text-[var(--color-text-primary)]">
              <option value="all">All Environments</option>
              <option value="production">Production</option>
              <option value="staging">Staging</option>
              <option value="development">Development</option>
            </select>
            <select value={filterType} onChange={e => setFilterType(e.target.value as GateType | "all")} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-1.5 text-sm text-[var(--color-text-primary)]">
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="font-semibold text-[var(--color-text-primary)]">{gate.name}</span>
                    ...
                    <div className="text-xs text-[var(--color-text-muted)] font-mono">{gate.key}</div>
                    <div className="text-xs text-[var(--color-text-secondary)] mt-1">{gate.description}</div>
                  </div>
                  <div className="text-right">
                    {gate.type === "percentage" && (
                      <div className="text-2xl font-bold text-[var(--color-text-primary)]">{gate.rolloutPercent}%</div>
                    )}
                    {gate.type === "boolean" && (
                      <div className={cn("text-2xl font-bold", gate.status === "enabled" ? "text-emerald-400" : "text-[var(--color-text-muted)]")}>
                ...
                <div className="flex items-center justify-between text-xs text-[var(--color-text-muted)] mt-2">
                  ...
                  <div className="flex gap-1">
                    {gate.tags.map(t => <span key={t} className="text-xs px-1.5 py-0.5 rounded bg-[var(--color-surface-2)] text-[var(--color-text-secondary)]">{t}</span>)}
                  </div>
                </div>
              </div>
            ))}
      ...
      {tab === "gates" && selectedGate && (
        <div>
          <button onClick={() => setSelectedGate(null)} className="flex items-center gap-1 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] mb-4 transition-colors">
            ← Back to gates
          </button>
          ...
          <div className="grid md:grid-cols-2 gap-5 mb-5">
            <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)]">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h2 className="text-xl font-bold text-[var(--color-text-primary)]">{selectedGate.name}</h2>
                    <span className={cn("text-xs px-2 py-0.5 rounded-full capitalize", statusBadge(selectedGate.status))}>{selectedGate.status}</span>
                  </div>
                  <div className="text-xs text-[var(--color-text-secondary)] font-mono mb-2">{selectedGate.key}</div>
                ...
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 text-xs bg-emerald-600 hover:bg-emerald-500 rounded-lg text-[var(--color-text-primary)] transition-colors">Enable</button>
                  <button className="px-3 py-1.5 text-xs bg-[var(--color-surface-3)] hover:bg-[var(--color-surface-3)] rounded-lg text-[var(--color-text-primary)] transition-colors">Disable</button>
                </div>
              </div>
              <p className="text-sm text-[var(--color-text-primary)] mb-4">{selectedGate.description}</p>
            ...
            <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)]">
              <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Evaluation Stats</h3>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-[var(--color-surface-2)] rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-[var(--color-text-primary)]">{selectedGate.evaluations.toLocaleString()}</div>
                  <div className="text-xs text-[var(--color-text-muted)]">total evaluations</div>
                </div>
                <div className="bg-[var(--color-surface-2)] rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-emerald-400">{selectedGate.truePercent}%</div>
                  <div className="text-xs text-[var(--color-text-muted)]">returning true</div>
                </div>
              </div>
              {selectedGate.type === "percentage" && (
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-[var(--color-text-secondary)]">Rollout</span>
              ...
              {selectedGate.schedule && (
                <div className="mt-3 p-3 bg-orange-500/10 rounded-lg border border-orange-500/20">
                  ...
                  <div className="text-xs text-[var(--color-text-primary)]">{selectedGate.schedule.start} → {selectedGate.schedule.end}</div>
                  <div className="text-xs text-[var(--color-text-muted)]">{selectedGate.schedule.timezone}</div>
                </div>
              )}
            </div>
          </div>
          ...
          {(selectedGate.targetSegments.length > 0 || selectedGate.targetUsers.length > 0) && (
            <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)] mb-5">
              <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Targeting</h3>
              {selectedGate.targetSegments.length > 0 && (
                <div className="mb-4">
                  <div className="text-xs text-[var(--color-text-muted)] mb-2">Segments</div>
                  <div className="space-y-2">
                    {selectedGate.targetSegments.map(sid => {
                      const seg = segmentMap[sid];
                      return seg ? (
                        <div key={sid} className="flex justify-between items-center p-3 bg-[var(--color-surface-2)] rounded-lg">
                          <div>
              ...
              {selectedGate.targetUsers.length > 0 && (
                <div>
                  <div className="text-xs text-[var(--color-text-muted)] mb-2">Specific Users</div>
                  <div className="flex flex-wrap gap-2">
                    {selectedGate.targetUsers.map(u => (
                      <span key={u} className="text-xs px-2 py-1 bg-[var(--color-surface-2)] rounded-lg text-[var(--color-text-primary)]">{u}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {selectedGate.dependsOn.length > 0 && (
            <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)]">
              <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-3">Dependencies</h3>
              <div className="flex flex-wrap gap-2">
      ...
      {tab === "segments" && (
        <div className="space-y-3">
          {SEGMENTS.map(seg => {
            ...
            return (
              <div key={seg.id} className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="font-medium text-[var(--color-text-primary)]">{seg.name}</div>
      ...
      {tab === "analytics" && (
        <div className="space-y-6">
          {/* Evaluation chart */}
          <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)]">
            <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Hourly Evaluations (Today)</h3>
                ...
                return (
                  <div key={pt.time} className="flex flex-col items-center gap-1 flex-1">
                    <div className="w-full flex items-end" style={{ height: 80 }}>
                      <div className="w-full relative rounded-t overflow-hidden bg-[var(--color-surface-3)]" style={{ height: `${totalPct}%`, minHeight: 4 }}>
            ...
            <div className="flex gap-4 mt-3 text-xs text-[var(--color-text-muted)]">
              ...
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-[var(--color-surface-3)] inline-block" />False</span>
            </div>
          </div>

          {/* Per-gate usage */}
          <div className="bg-[var(--color-surface-1)] rounded-xl p-5 border border-[var(--color-border)]">
            <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Gate Evaluation Volume</h3>
            <div className="space-y-3">
              {[...GATES].toSorted((a, b) => b.evaluations - a.evaluations).map(g => {
                ...
                return (
                  <div key={g.id}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-[var(--color-text-primary)] font-mono">{g.key}</span>
      ...
      {tab === "events" && (
        <div className="space-y-3">
          {EVENTS.map(ev => (
            <div key={ev.id} className="bg-[var(--color-surface-1)] rounded-xl p-4 border border-[var(--color-border)]">
              ...
              <div className="text-xs text-[var(--color-text-muted)] mb-1">by {ev.user}</div>
              <div className="text-sm text-[var(--color-text-primary)]">{ev.detail}</div>
            </div>
          ))}
        </div>
      )}
