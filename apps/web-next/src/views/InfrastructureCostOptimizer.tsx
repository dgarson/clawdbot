...
function typeColor(t: ResourceType): string {
  const map: Record<ResourceType, string> = {
    ...
    elb:          "bg-[var(--color-surface-3)]/20 text-[var(--color-text-secondary)] border-[var(--color-surface-3)]/30",
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Monthly Spend", value: "$" + totalCurrent.toLocaleString(), color: "text-[var(--color-text-primary)]" },
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-5">
        <h3 className="text-sm font-semibold text-[var(--color-text-primary)] mb-4">Cost by Category: Current vs Optimized</h3>
              ...
              <div key={cat.name}>
                ...
                <div className="relative">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-[var(--color-surface-2)] rounded h-3 overflow-hidden">
                      <div className="h-full bg-[var(--color-surface-3)] relative" style={{ width: (cat.current / maxCost * 100) + "%" }}>
                        <div className="absolute inset-0 bg-emerald-500" style={{ width: (cat.optimized / cat.current * 100) + "%" }} />
                      </div>
                    </div>
                    <span className="text-xs text-[var(--color-text-secondary)] w-16 text-right">${cat.current.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="flex gap-4 mt-3 text-xs">
          <div className="flex items-center gap-1"><div className="w-3 h-2 rounded bg-emerald-500" /><span className="text-[var(--color-text-secondary)]">Optimized</span></div>
          <div className="flex items-center gap-1"><div className="w-3 h-2 rounded bg-[var(--color-surface-3)]" /><span className="text-[var(--color-text-secondary)]">Current overspend</span></div>
        </div>
      </div>
    </div>
  );
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="font-medium text-[var(--color-text-primary)]">{op.title}</span>
                <span className="text-xs bg-[var(--color-surface-2)] text-[var(--color-text-secondary)] rounded px-2 py-0.5 capitalize">{op.type}</span>
              </div>
              <p className="text-xs text-[var(--color-text-muted)] mt-0.5">{op.description}</p>
            </div>
            <div className="text-right shrink-0">
              <div className="text-emerald-400 font-bold">${op.monthlySavings.toLocaleString()}/mo</div>
              <div className="text-xs text-[var(--color-text-muted)]">{op.resourceCount} resources</div>
            </div>
          </div>
          {selected?.id === op.id && (
            <div className="mt-4 border-t border-[var(--color-border)] pt-4 grid grid-cols-4 gap-4 text-xs">
              <div>
                <div className="text-[var(--color-text-muted)]">Monthly</div>
                <div className="text-emerald-400 font-bold">${op.monthlySavings.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-[var(--color-text-muted)]">Annual</div>
                <div className="text-emerald-400 font-bold">${op.annualSavings.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-[var(--color-text-muted)]">Effort</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {visible.map((res) => (
          <div key={res.id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-4">
            <div className="flex items-center gap-3">
              <span className={cn("text-xs px-1.5 py-0.5 rounded border uppercase", typeColor(res.type))}>{res.type}</span>
              <span className="font-medium text-[var(--color-text-primary)] text-sm">{res.name}</span>
              <span className="text-xs text-[var(--color-text-muted)]">{res.region}</span>
              <span className="ml-auto text-sm font-semibold text-[var(--color-text-primary)]">${res.monthlyCost.toFixed(0)}/mo</span>
              {res.wasteCost > 0 && (
                <span className="text-xs text-rose-400">waste: ${res.wasteCost.toFixed(0)}</span>
              )}
            </div>
            <div className="mt-2 flex items-center gap-3">
              <span className="text-xs text-[var(--color-text-muted)] w-24">Utilization</span>
