...
const driftStatusBg: Record<DriftStatus, string> = {
  ...
  unknown: "bg-[var(--color-surface-3)]/30 border-[var(--color-surface-3)]",
...
const historyStatusColor: Record<string, string> = {
  ...
  ignored: "text-[var(--color-text-muted)]",
};
    ...
    <div className="flex flex-col h-full bg-[var(--color-surface-0)] text-[var(--color-text-primary)]">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
        <div>
          ...
          <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Comparing live state vs Terraform / Helm expected state</p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-0 border-b border-[var(--color-border)]">
        {[
          ...
          <div key={i} className={cn("px-6 py-3 border-r border-[var(--color-border)] last:border-r-0", stat.alert && "bg-rose-500/5")}>
            <div className={cn("text-xl font-bold", stat.alert ? "text-rose-400" : "text-[var(--color-text-primary)]")}>{stat.value}</div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        {tab === "overview" && (
          <div className="p-6 space-y-6">
            ...
            <div>
              <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Drifted Resources</h2>
              <div className="space-y-2">
                {DRIFT_RESOURCES.filter(r => r.status === "drifted").map(resource => (
                  <div
                    key={resource.id}
                    className="bg-[var(--color-surface-1)] rounded-lg border border-[var(--color-border)] hover:border-[var(--color-border)] p-4 cursor-pointer transition-colors"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-sm text-[var(--color-text-primary)]">{resource.name}</span>
                            {resource.severity && (
                          ...
                          <p className="text-xs text-[var(--color-text-muted)]">{resource.type} · {resource.region}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-medium text-rose-400">{resource.driftCount} drift{resource.driftCount !== 1 ? "s" : ""}</div>
                        <div className="text-xs text-[var(--color-text-muted)]">vs {resource.expectedBy}</div>
                      </div>
                    </div>
                    <div className="mt-3 space-y-1">
                      {resource.drifts.slice(0, 2).map((drift, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs font-mono">
                          ...
                          <span className="text-[var(--color-text-secondary)]">{drift.field}:</span>
                      ...
                      {resource.drifts.length > 2 && (
                        <div className="text-xs text-[var(--color-text-muted)]">+{resource.drifts.length - 2} more drift points</div>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div>
              <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Drift by Provider</h2>
              <div className="grid grid-cols-4 gap-3">
                {(["aws", "gcp", "azure", "k8s"] as const).map(provider => {
                  ...
                  return (
                    <div key={provider} className="bg-[var(--color-surface-1)] rounded-lg border border-[var(--color-border)] p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-2xl">{providerIcon[provider]}</span>
                        <span className="text-sm font-medium text-[var(--color-text-primary)] uppercase">{provider}</span>
                      </div>
                      <div className="h-1.5 bg-[var(--color-surface-2)] rounded-full overflow-hidden mb-1">
                        <div
                          className="h-full bg-rose-500 rounded-full"
                          style={{ width: total > 0 ? `${(drifted / total) * 100}%` : "0%" }}
                        />
                      </div>
                      <div className="text-xs text-[var(--color-text-muted)]">{drifted}/{total} resources drifted</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="flex h-full">
                ...
                {filteredResources.map(resource => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedResource?.id === resource.id && "bg-[var(--color-surface-2)]"
                    )}
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      <div className="flex items-center gap-2">
                        <span className="text-base">{providerIcon[resource.provider]}</span>
                        <span className="text-xs font-mono text-[var(--color-text-primary)] truncate max-w-[180px]">{resource.name}</span>
                      </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {selectedResource ? (
                <div className="p-6 space-y-6">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <div>
                        <h2 className="font-mono text-sm font-semibold text-[var(--color-text-primary)]">{selectedResource.name}</h2>
                        <p className="text-xs text-[var(--color-text-muted)]">{selectedResource.type} · {selectedResource.region}</p>
                      </div>
                  ...
                  {selectedResource.drifts.length > 0 ? (
                    <div>
                      <div className="text-xs text-[var(--color-text-muted)] mb-3 uppercase tracking-wider">Drift Details</div>
                      <div className="space-y-2">
                        {selectedResource.drifts.map((drift, i) => (
                          <div key={i} className={cn(
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="font-mono text-sm text-[var(--color-text-primary)]">{drift.field}</span>
                            </div>
                            <div className="grid grid-cols-2 gap-3 font-mono text-xs">
                              <div className="bg-emerald-500/5 border border-emerald-500/20 rounded p-2">
                                <div className="text-[var(--color-text-muted)] mb-1">Expected (IaC)</div>
                                <div className="text-emerald-400">{drift.expected}</div>
                              </div>
                              <div className="bg-rose-500/5 border border-rose-500/20 rounded p-2">
                                <div className="text-[var(--color-text-muted)] mb-1">Actual (live)</div>
                    ...
                    <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-4 text-center">
                      ...
                      <p className="text-xs text-[var(--color-text-muted)] mt-1">Resource matches expected state</p>
                    </div>
                  )}
                ...
                <div className="flex items-center justify-center h-full text-[var(--color-text-muted)] text-sm">Select a resource to view drift details</div>
              )}
            </div>
          </div>
        )}
            ...
            <div className="space-y-2">
              {DRIFT_HISTORY.map(event => (
                <div key={event.id} className={cn(
                  "bg-[var(--color-surface-1)] rounded-lg border px-5 py-3 flex items-center gap-4",
                  event.status === "detected" ? "border-rose-500/20" : "border-[var(--color-border)]"
                )}>
                  <span className="text-lg shrink-0">{event.status === "remediated" ? "✅" : event.status === "acknowledged" ? "👁️" : event.status === "ignored" ? "🔕" : "🚨"}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm text-[var(--color-text-primary)]">{event.resourceName}</span>
                      <span className="text-xs text-[var(--color-text-muted)]">{event.type}</span>
                    </div>
                    <div className="text-xs text-[var(--color-text-muted)] mt-0.5">
                      {event.driftCount} drift{event.driftCount !== 1 ? "s" : ""} detected
                      {event.remediatedBy && <span> · remediated by <span className="text-[var(--color-text-secondary)]">{event.remediatedBy}</span></span>}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={cn("text-xs font-medium", historyStatusColor[event.status])}>{event.status}</div>
                    <div className="text-xs text-[var(--color-text-muted)] mt-0.5">{event.timestamp}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === "remediation" && (
          <div className="p-6 space-y-6">
            <div>
              <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-4">Auto-Remediation Plan</h2>
              <div className="bg-[var(--color-surface-1)] rounded-lg border border-[var(--color-border)] p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                ...
                <div className="space-y-1">
                  {DRIFT_RESOURCES.filter(r => r.status === "drifted").map(r => (
                    <div key={r.id} className="flex items-center gap-2 text-xs font-mono text-[var(--color-text-muted)] py-0.5">
                      <span className="text-amber-400">~</span>
                      <span className="text-[var(--color-text-primary)]">{r.type.toLowerCase().replace(/ /g, "_")}</span>
                ...
                <div className="bg-[var(--color-surface-2)] rounded p-3 font-mono text-xs text-[var(--color-text-secondary)]">
                  <div className="text-[var(--color-text-muted)]">$ terraform plan -out=drift-fix.tfplan</div>
                  <div className="text-emerald-400 mt-1">Plan: 0 to add, 7 to change, 0 to destroy.</div>
                  <div className="text-[var(--color-text-muted)] mt-0.5">Changes to Outputs: (none)</div>
                </div>
              </div>
            </div>

            <div>
              <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-4">Remediation Settings</h2>
              <div className="space-y-3">
                {[
                  ...
                  <div key={setting.name} className="flex items-center justify-between bg-[var(--color-surface-1)] rounded-lg border border-[var(--color-border)] px-4 py-3">
                    <div>
                      <div className="text-sm font-medium text-[var(--color-text-primary)]">{setting.name}</div>
