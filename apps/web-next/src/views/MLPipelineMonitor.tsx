...
const pipelineStatusBg: Record<PipelineStatus, string> = {
  ...
  cancelled: "bg-zinc-700/30 border-zinc-600/30",
};
    ...
    <div className="flex flex-col h-full bg-zinc-950 text-white">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">MLflow + Ray cluster · us-east-1</p>
        </div>
      ...
      <div className="flex-1 overflow-auto">
              ...
              {PIPELINES.map(pipeline => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedPipeline?.id === pipeline.id && "bg-zinc-800"
                  )}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-zinc-200 font-mono">{pipeline.name}</span>
                  ...
                  <p className="text-xs text-zinc-500 mb-2 line-clamp-1">{pipeline.description}</p>
                  {/* Stage pills */}
                  <div className="flex items-center gap-1">
                    {pipeline.stages.map((stage, i) => (
                      <React.Fragment key={stage.name}>
                        <div className={cn("h-1.5 rounded-full flex-1", stageStatusColor[stage.status])} title={stage.name} />
                        {i < pipeline.stages.length - 1 && <div className="w-0.5 h-0.5 rounded-full bg-zinc-700" />}
                      </React.Fragment>
                    ))}
                  </div>
                ...
                <div className="p-6 space-y-6">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h2 className="font-mono text-sm font-semibold text-white">{selectedPipeline.name}</h2>
                    ...
                    <p className="text-xs text-zinc-400">{selectedPipeline.description}</p>
                  </div>

                  {/* Stage progress */}
                  <div>
                    <div className="text-xs text-zinc-500 mb-3 uppercase tracking-wider">Pipeline Stages</div>
                        ...
                        <div key={stage.name} className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              {stage.duration && <span className="text-xs text-zinc-600">{stage.duration}</span>}
                            </div>
                            {stage.status === "running" && (
                  ...
                  <div className="flex gap-2">
                    {selectedPipeline.status === "running" ? (
                      ...
                      <button className="px-3 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 rounded text-white transition-colors">Trigger Run</button>
                    )}
                    {["View Logs", "Edit Config", "Clone Pipeline"].map(action => (
                      <button key={action} className="px-3 py-1.5 text-xs bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded text-zinc-300 transition-colors">{action}</button>
                    ))}
                  </div>
                  ...
                  {selectedPipeline.status === "failed" && (
                    <div className="bg-rose-500/10 border border-rose-500/30 rounded-lg p-4">
                      ...
                      <div className="mt-2 flex gap-2">
                        ...
                        <button className="px-3 py-1 text-xs bg-zinc-800 border border-zinc-700 rounded text-zinc-400 hover:bg-zinc-700 transition-colors">Retry with lower batch size</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {tab === "runs" && (
          <div className="p-6">
            <div className="space-y-2">
              {RUNS.map(run => (
                <div key={run.id} className="bg-zinc-900 rounded-lg border border-zinc-800 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs text-zinc-500">{run.runId}</span>
                        <span className="text-sm font-medium text-zinc-200">{run.pipelineName}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      {run.gpuCount > 0 && (
                        <span className="text-xs text-zinc-500">{run.gpuCount}× {run.gpuType}</span>
                      )}
                    ...
                    <div className="flex gap-4 flex-wrap">
                      {run.trainLoss !== null && (
                        <div className="text-xs">
                          <span className="text-zinc-600">train_loss </span>
                          <span className="text-zinc-300 font-mono">{run.trainLoss.toFixed(4)}</span>
                        </div>
                      )}
                      {run.valLoss !== null && (
                        <div className="text-xs">
                          <span className="text-zinc-600">val_loss </span>
                          <span className="text-zinc-300 font-mono">{run.valLoss.toFixed(4)}</span>
                        </div>
                      )}
                      {run.accuracy !== null && (
                        <div className="text-xs">
                      ...
                      {Object.entries(run.metrics).map(([k, v]) => (
                        <div key={k} className="text-xs">
                          ...
                          <span className="text-zinc-300 font-mono">{typeof v === "number" && v < 10 ? v.toFixed(3) : v}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="flex h-full">
            <div className="w-80 border-r border-zinc-800 overflow-y-auto">
              ...
              {MODEL_VERSIONS.map(model => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-zinc-200 font-mono">{model.name}</span>
            ...
            <div className="flex-1 overflow-y-auto">
              {selectedModel ? (
                <div className="p-6 space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="font-mono text-sm font-semibold text-white">{selectedModel.name} <span className="text-zinc-500">v{selectedModel.version}</span></h2>
                      <p className="text-xs text-zinc-500 mt-0.5">{selectedModel.framework} · created {selectedModel.createdAt} · run <span className="font-mono">{selectedModel.runId}</span></p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={label} className="bg-zinc-900 rounded p-4 text-center">
                  ...
                  <div>
                    <div className="text-xs text-zinc-500 mb-3 uppercase tracking-wider">Evaluation Metrics</div>
                    <div className="space-y-2">
                      {Object.entries(selectedModel.metrics).map(([key, value]) => (
                        <div key={key} className="flex items-center gap-3">
                          <span className="text-xs text-zinc-400 w-28 font-mono shrink-0">{key}</span>
                          ...
                          <span className="text-xs text-zinc-300 font-mono w-12 text-right">{value < 10 ? value.toFixed(3) : value.toFixed(0)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {selectedModel.stage === "candidate" && (
                      <button className="px-3 py-1.5 text-xs bg-emerald-600 hover:bg-emerald-500 rounded text-white transition-colors">Promote to Production</button>
                    )}
                ...
                <div className="flex items-center justify-center h-full text-zinc-600 text-sm">Select a model version</div>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "compute" && (
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-semibold text-zinc-300">Compute Nodes</h2>
              <div className="flex items-center gap-3 text-xs text-zinc-500">
                ...
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-zinc-600 inline-block" /> Offline</span>
              </div>
            </div>

            <div className="space-y-3">
              {COMPUTE.map(node => (
                <div key={node.id} className="bg-zinc-900 rounded-lg border border-zinc-800 p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "w-2.5 h-2.5 rounded-full",
                        node.status === "available" ? "bg-emerald-400" :
                        node.status === "busy" ? "bg-sky-400 animate-pulse" : "bg-zinc-600"
                      )} />
                      <div>
                  ...
                  {node.gpuCount > 0 ? (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-zinc-500">GPU Utilization</span>
                          <span className={cn("text-xs font-medium", node.gpuUtil > 80 ? "text-amber-400" : "text-zinc-300")}>{node.gpuUtil}%</span>
                        </div>
                        ...
                        <div className="text-xs text-zinc-600 mt-1">{node.gpuCount} GPUs</div>
                      </div>
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-zinc-500">Memory</span>
                          <span className="text-xs font-medium text-zinc-300">{node.memUtil}%</span>
                        </div>
                        <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                          <div
                        ...
                        <div className="text-xs text-zinc-600 mt-1">GPU VRAM</div>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-zinc-500">CPU Memory</span>
                        <span className="text-xs font-medium text-zinc-300">{node.memUtil}%</span>
                      </div>
                      <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                        <div
                  ...
                  {node.status === "offline" && (
                    <div className="mt-3 flex items-center gap-2 text-xs text-zinc-500 bg-zinc-800/50 rounded p-2">
