...
export default function PermissionsManager() {
  ...
  return (
    <div className="flex h-full w-full bg-zinc-950 text-white overflow-hidden font-sans">
      {/* Sidebar */}
      <aside className="w-64 border-r border-zinc-800 flex flex-col bg-zinc-950/50" role="navigation" aria-label="Agents List">
        <div className="p-4 border-b border-zinc-800">
          <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest">Agents</h2>
        </div>
          ...
          {agents.map(agent => (
            <button
              ...
              className={cn(
                ...
                selectedAgentId === agent.agentId ? "bg-zinc-900 shadow-inner" : "hover:bg-zinc-900/50"
              )}
      ...
      {/* Main Panel */}
      <main className="flex-1 flex flex-col min-w-0 bg-zinc-950" role="tabpanel" aria-label={`Permissions for ${selectedAgent.agentName}`}>
        {/* Header */}
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-900/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Last modified: {new Date(selectedAgent.lastModified).toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!isEditing ? (
              <button 
                ...
                className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-md text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none border border-zinc-700"
              >
                Edit Permissions
              </button>
            ) : (
              <>
                <button 
                  ...
                  className="px-4 py-2 bg-transparent hover:bg-zinc-800 rounded-md text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none text-zinc-400"
                >
                  Cancel
                </button>
                <button 
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="max-w-5xl mx-auto">
            <table className="w-full border-collapse text-left bg-zinc-900/50 rounded-lg overflow-hidden border border-zinc-800">
              <thead>
                <tr className="bg-zinc-900 border-b border-zinc-800">
                  <th scope="col" className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Resource Type</th>
                  {ACTIONS.map(action => (
                    <th key={action} scope="col" className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-center">{action}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {RESOURCE_TYPES.map(resType => (
                  <tr key={resType} className="hover:bg-zinc-800/30 transition-colors">
                    <th scope="row" className="p-4 font-medium text-zinc-300 capitalize">
                      {resType.replace("-", " ")}
                    </th>
                    {ACTIONS.map(action => {
                          ...
                          <div className="flex justify-center items-center">
                            ...
                            <div className="relative flex items-center">
                              <input
                                ...
                                className={cn(
                                  "h-5 w-5 rounded border-zinc-700 bg-zinc-800 text-indigo-600 transition-all",
                              ...
                              />
                              {inherited && (
                                <span className="absolute -top-1 -right-1 bg-zinc-950 rounded-full p-0.5" title="Inherited from role">
                                  <svg className="w-2.5 h-2.5 text-zinc-500" fill="currentColor" viewBox="0 0 20 20">
            ...
            {/* Legend / Status */}
            <div className="mt-8 flex flex-wrap gap-6 text-xs text-zinc-500 border-t border-zinc-800 pt-6">
              <div className="flex items-center gap-2">
                <div className="h-4 w-4 rounded bg-indigo-600 border border-indigo-500"></div>
                <span>Granted Permission</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-4 w-4 rounded bg-zinc-800 border border-zinc-700"></div>
