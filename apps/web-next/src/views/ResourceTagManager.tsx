...
const complianceColor: Record<ComplianceStatus, string> = {
  ...
  untagged: "text-[var(--color-text-secondary)] bg-[var(--color-surface-3)]/10",
}
...
function TagBadge({ tag }: { tag: Tag }) {
  return (
    <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-[var(--color-surface-2)] border border-[var(--color-border)]">
      <span className="text-[var(--color-text-secondary)]">{tag.key}:</span>
      <span className="text-[var(--color-text-primary)]">{tag.value}</span>
    </span>
  )
}
    ...
    <div className="min-h-screen bg-[var(--color-surface-0)] text-[var(--color-text-primary)] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-sm mt-1">Enforce tagging policies across AWS, GCP, and Azure</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4 cursor-pointer hover:border-emerald-500/30 transition-colors" onClick={() => setComplianceFilter(complianceFilter === "compliant" ? "all" : "compliant")}>
          <div className="text-xs text-[var(--color-text-secondary)] mb-1">Compliant</div>
          <div className="text-2xl font-bold text-emerald-400">{compliantCount}</div>
          <div className="text-xs text-[var(--color-text-muted)] mt-1">{Math.round((compliantCount / RESOURCES.length) * 100)}% of resources</div>
        </div>
        <div className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4 cursor-pointer hover:border-amber-500/30 transition-colors" onClick={() => setComplianceFilter(complianceFilter === "missing-required" ? "all" : "missing-required")}>
          <div className="text-xs text-[var(--color-text-secondary)] mb-1">Missing Tags</div>
          <div className="text-2xl font-bold text-amber-400">{missingCount}</div>
          <div className="text-xs text-[var(--color-text-muted)] mt-1">click to filter</div>
        </div>
        <div className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4 cursor-pointer hover:border-rose-500/30 transition-colors" onClick={() => setComplianceFilter(complianceFilter === "untagged" ? "all" : "untagged")}>
          <div className="text-xs text-[var(--color-text-secondary)] mb-1">Untagged</div>
          <div className="text-2xl font-bold text-rose-400">{untaggedCount + invalidCount}</div>
          <div className="text-xs text-[var(--color-text-muted)] mt-1">{invalidCount} invalid values</div>
        </div>
      ...
      {tab === "resources" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <select value={providerFilter} onChange={e => setProviderFilter(e.target.value)} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md px-3 py-1.5 text-sm text-[var(--color-text-primary)]">
              <option value="all">All Providers</option>
              <option value="aws">AWS</option>
              <option value="gcp">GCP</option>
              <option value="azure">Azure</option>
            </select>
            <select value={complianceFilter} onChange={e => setComplianceFilter(e.target.value)} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md px-3 py-1.5 text-sm text-[var(--color-text-primary)]">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  ...
                  <th className="text-left p-3 text-[var(--color-text-secondary)] font-medium">Resource</th>
                  <th className="text-left p-3 text-[var(--color-text-secondary)] font-medium">Tags</th>
                  <th className="text-left p-3 text-[var(--color-text-secondary)] font-medium">Cost/mo</th>
                  <th className="text-left p-3 text-[var(--color-text-secondary)] font-medium">Compliance</th>
                  <th className="text-left p-3 text-[var(--color-text-secondary)] font-medium">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(r => (
                  <React.Fragment key={r.id}>
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[var(--color-text-muted)] font-mono">{r.id}</div>
                            <div className="text-xs mt-0.5">
                              <span className={cn("font-medium uppercase", providerColor[r.provider])}>{r.provider}</span>
                              <span className="text-[var(--color-text-muted)] ml-1">{r.region}</span>
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="p-3">
                        {r.tags.length > 0 ? (
                          <div className="flex flex-wrap gap-1 max-w-xs">
                            ...
                            {r.tags.length > 3 && (
                              <span className="text-xs text-[var(--color-text-muted)] self-center">+{r.tags.length - 3}</span>
                            )}
                          </div>
                        ) : (
                          <span className="text-xs text-[var(--color-text-muted)] italic">No tags</span>
                        )}
                      </td>
                      <td className="p-3 text-[var(--color-text-primary)] font-mono text-sm">${r.monthlyCost.toLocaleString()}</td>
                      <td className="p-3">
                    ...
                    {selectedResource === r.id && (
                      <tr className="border-b border-[var(--color-border)]/50">
                        <td colSpan={6} className="p-4 bg-[var(--color-surface-0)]">
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {r.tags.map(t => (
                                <div key={t.key} className="flex items-center gap-2 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md px-3 py-2">
                                  <input defaultValue={t.key} className="bg-transparent text-[var(--color-text-secondary)] text-xs w-24 outline-none" />
                                  <span className="text-[var(--color-text-muted)]">:</span>
                                  <input defaultValue={t.value} className="bg-transparent text-[var(--color-text-primary)] text-xs flex-1 outline-none" />
                                  <button className="text-[var(--color-text-muted)] hover:text-rose-400 transition-colors text-xs">✕</button>
                                </div>
                              ))}
                              <button className="flex items-center gap-2 bg-[var(--color-surface-2)]/50 border border-[var(--color-border)] border-dashed rounded-md px-3 py-2 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors">
                                + Add Tag
                              </button>
                            </div>
                            <div className="flex gap-2 pt-1">
                              <span className="text-xs text-[var(--color-text-muted)]">Apply template:</span>
                              {TEMPLATES.slice(0, 3).map(t => (
                                <button key={t.id} className="text-xs px-2 py-0.5 bg-[var(--color-surface-2)] hover:bg-[var(--color-surface-3)] rounded text-[var(--color-text-secondary)] transition-colors">
                                  {t.name}
                                </button>
                              ))}
                            </div>
      ...
      {tab === "policies" && (
        <div className="space-y-4">
          {POLICIES.map(pol => (
            <div key={pol.id} className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-[var(--color-text-secondary)] mb-3">{pol.description}</p>
                  <div className="space-y-2">
                    <div>
                      <span className="text-xs text-[var(--color-text-muted)] uppercase tracking-wider">Required Keys</span>
                      <div className="flex gap-1.5 mt-1">
                        {pol.requiredKeys.map(k => (
                          <span key={k} className="text-xs px-2 py-0.5 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded font-mono text-[var(--color-text-primary)]">{k}</span>
                        ))}
                      </div>
                    </div>
                    {Object.keys(pol.allowedValues).length > 0 && (
                      <div>
                        <span className="text-xs text-[var(--color-text-muted)] uppercase tracking-wider">Allowed Values</span>
                        <div className="mt-1 space-y-1">
                          {Object.entries(pol.allowedValues).map(([key, vals]) => (
                            <div key={key} className="flex items-center gap-2 text-xs">
                              <span className="font-mono text-[var(--color-text-secondary)] w-24">{key}:</span>
                              <div className="flex gap-1 flex-wrap">
                                {vals.map(v => (
                                  <span key={v} className="px-1.5 py-0.5 bg-[var(--color-surface-2)] rounded text-[var(--color-text-primary)]">{v}</span>
                                ))}
                              </div>
                            </div>
                          ))}
                    ...
                    <div>
                      <span className="text-xs text-[var(--color-text-muted)] uppercase tracking-wider">Applies To</span>
                      <div className="flex gap-1.5 mt-1">
                        {pol.appliesTo.map(t => (
                          <span key={t} className="text-xs px-2 py-0.5 bg-[var(--color-surface-2)] rounded text-[var(--color-text-secondary)]">
                            {typeIcon[t]} {t}
                          </span>
                        ))}
                      </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-xs text-[var(--color-text-secondary)]">violations</div>
                  <div className="flex flex-col gap-1 mt-2">
                    <button className="px-3 py-1 text-xs bg-[var(--color-surface-2)] hover:bg-[var(--color-surface-3)] rounded-md text-[var(--color-text-primary)] transition-colors">Edit</button>
                    {pol.violations > 0 && (
      ...
      {tab === "templates" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {TEMPLATES.map(tmpl => (
              <div key={tmpl.id} className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4">
                ...
                <div className="space-y-1.5 mt-3">
                  {tmpl.tags.map(t => (
                    <div key={t.key} className="flex items-center gap-2 text-xs bg-[var(--color-surface-2)] rounded px-2 py-1.5">
                      <span className="text-[var(--color-text-secondary)] font-mono w-28">{t.key}</span>
                      <span className="text-[var(--color-text-muted)]">→</span>
                      <span className={cn("font-mono flex-1", t.value ? "text-[var(--color-text-primary)]" : "text-[var(--color-text-muted)] italic")}>
                        {t.value || "(fill in)"}
                      </span>
                    </div>
                  ))}
      ...
      {tab === "analytics" && (
        <div className="space-y-6">
          {/* Tag key coverage */}
          <div className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4">
            <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Tag Key Coverage (resources using each key)</h3>
            <div className="space-y-2.5">
              {topKeys.map(([key, count]) => (
                <div key={key} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-mono text-[var(--color-text-primary)]">{key}</span>
                    <span className="text-[var(--color-text-secondary)]">{count} / {RESOURCES.length} resources ({Math.round((count / RESOURCES.length) * 100)}%)</span>
                  </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {(["aws", "gcp", "azure"] as const).map(provider => {
              ...
              return (
                <div key={provider} className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4">
                  <div className={cn("text-sm font-bold uppercase mb-1", providerColor[provider])}>{provider}</div>
                  <div className="text-2xl font-bold text-[var(--color-text-primary)] mb-2">{pResources.length}</div>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      <span className="text-[var(--color-text-secondary)]">Compliant</span>
                      <span className="text-emerald-400">{pCompliant}/{pResources.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[var(--color-text-secondary)]">Monthly cost</span>
                      <span className="text-[var(--color-text-primary)]">${pCost.toLocaleString()}</span>
                    </div>
                  </div>
                  <div className="mt-2 w-full h-1.5 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
          ...
          {/* Cost attribution */}
          <div className="bg-[var(--color-surface-1)] border border-[var(--color-border)] rounded-lg p-4">
            <h3 className="text-sm font-medium text-[var(--color-text-primary)] mb-4">Monthly Cost by Compliance Status</h3>
            <div className="space-y-2">
                ...
                return (
                  <div key={status} className="space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[var(--color-text-primary)]">${cost.toLocaleString()}/mo</span>
                    </div>
                    <div className="w-full h-2 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
                      <div
