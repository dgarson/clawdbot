      ...
      <div className="flex-shrink-0 px-6 py-4 border-b border-zinc-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-5">
            <div>
              ...
              <p className="text-sm text-zinc-400">Engineering health across 6 dimensions</p>
            </div>
            {/* Overall score */}
            ...
            {(["pillars", "agents"] as const).map((v) => (
              <button
                ...
                className={cn(
                  ...
                  view === v ? "bg-indigo-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"
                )}
              >
        ...
        <div className="grid grid-cols-6 gap-2">
          {PILLARS.map((pillar) => {
            ...
            const trendColor = pillar.trend === "up" ? "text-emerald-400" : pillar.trend === "down" ? "text-rose-400" : "text-zinc-500";
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? gradeBg(pillar.grade) + " ring-1 ring-indigo-400/30" : "bg-zinc-900 border-zinc-800 hover:border-zinc-600"
                )}
              >
                <div className="text-lg mb-1">{pillar.emoji}</div>
                <div className={cn("text-2xl font-black", gradeColor(pillar.grade))}>{pillar.grade}</div>
                <div className="text-xs text-zinc-400 font-medium">{pillar.name}</div>
        ...
        {view === "pillars" && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-lg font-bold text-white">{selectedPillar.emoji} {selectedPillar.name}</h2>
                  <p className="text-sm text-zinc-400">{selectedPillar.recommendation}</p>
                </div>
                <div className={cn("px-4 py-2 rounded-xl border", gradeBg(selectedPillar.grade))}>
                  ...
                  <span className="text-zinc-400 text-sm ml-2">{selectedPillar.score}/100</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {selectedPillar.metrics.map((metric) => {
                  ...
                  const trendColor = metric.trend === "up" ? "text-emerald-400" : metric.trend === "down" ? "text-rose-400" : "text-zinc-500";
                  const maxHistory = Math.max(...metric.history);

                  return (
                    <div key={metric.id} className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-zinc-500">Target: {metric.target}{metric.unit}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-end gap-0.5 h-8">
                        {metric.history.map((v, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className={cn("flex-1 rounded-sm", isLast ? barColor : "bg-zinc-700")}
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-zinc-600">8 weeks ago</span>
                ...
                <div key={agent.agent} className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-zinc-500">{agent.squad}</div>
                    </div>
                    <div className={cn("flex items-center gap-2 px-3 py-1.5 rounded-xl border", gradeBg(agent.grade))}>
                      ...
                      <span className="text-white font-bold">{agent.overallScore}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-6 gap-2">
                    {PILLARS.map((pillar) => {
                      ...
                      return (
                        <div key={pillar.id} className="text-center">
                          ...
                          <div className="h-1 bg-zinc-800 rounded-full mt-0.5">
                            <div className={cn("h-full rounded-full", scoreBarColor(score))} style={{ width: `${score}%` }} />
                          </div>
                          <div className="text-[9px] text-zinc-600 mt-0.5">{pillar.name}</div>
                        </div>
                      );
                    })}
                  </div>
