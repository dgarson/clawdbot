    ...
    <div className="h-full flex flex-col bg-zinc-950 text-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 shrink-0">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Manage service accounts, API keys, and rotation policies</p>
        </div>
              ...
              {SERVICE_ACCOUNTS.map((sa) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedSA.id === sa.id ? "bg-indigo-600/10" : "hover:bg-zinc-800/40"
                  )}
                >
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-medium text-white">{sa.displayName}</div>
                    <div className={cn("text-[10px] px-1 py-0.5 rounded border", statusColor[sa.status])}>{sa.status}</div>
                  </div>
                  <div className="text-[10px] text-zinc-500 font-mono mt-1">{sa.name}</div>
                  <div className="text-[10px] text-zinc-400 mt-1">{sa.team}</div>
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-sm text-zinc-500 font-mono mt-1">{selectedSA.name}</div>
                  <p className="text-sm text-zinc-400 mt-2">{selectedSA.description}</p>
                </div>
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 text-xs border border-zinc-700 text-zinc-400 hover:border-zinc-600 rounded-md transition-colors">Edit</button>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
                <div className="text-xs text-zinc-400 font-semibold uppercase tracking-wider mb-3">Roles & Scopes</div>
                <div className="flex flex-wrap gap-2">
                  {selectedSA.roles.map((r) => (
                    <span key={r} className="text-xs px-2 py-1 rounded bg-zinc-800 border border-zinc-700 text-zinc-300 font-mono">{r}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800">
                  <div className="text-xs text-zinc-400 font-semibold uppercase tracking-wider">API Keys</div>
                      ...
                      <div key={key.id} className="px-5 py-4">
                        ...
                        {key.scopes.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {key.scopes.map((s) => (
                              <span key={s} className="text-[10px] px-1 py-0.5 rounded bg-zinc-800 text-zinc-500 font-mono">{s}</span>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          <div className="h-full overflow-y-auto p-6">
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
              <div className="px-5 py-3 border-b border-zinc-800 flex items-center justify-between">
                ...
                <span className="text-xs text-zinc-500">{allKeys.length} total</span>
              </div>
                    ...
                    {allKeys.map((key) => {
                      ...
                      return (
                        <tr key={key.id} className="hover:bg-zinc-800/30">
                          ...
                          <td className="px-4 py-3 text-xs text-zinc-400">{key.saName}</td>
                          <td className="px-4 py-3 text-xs font-mono text-zinc-300">{key.prefix}…</td>
                          ...
                          <td className="px-4 py-3 text-xs text-zinc-400">{key.lastUsed ? key.lastUsed.slice(0, 10) : "—"}</td>
                          <td className="px-4 py-3 text-xs text-zinc-300">{key.usageCount.toLocaleString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            {expiringSoon.length > 0 && (
              <div className="bg-amber-400/10 border border-amber-400/30 rounded-lg p-4">
                ...
                <div className="space-y-1">
                  {expiringSoon.map((k) => (
                    <div key={k.id} className="text-xs text-zinc-300">
                ...
                {SERVICE_ACCOUNTS.map((sa) => (
                  <div key={sa.id} className="px-5 py-4 flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-zinc-500 font-mono">{sa.name}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <select
                        defaultValue={sa.rotationPolicy}
                        className="bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300 focus:outline-none focus:border-indigo-500"
                      >
                ...
                <div key={entry.id} className="bg-zinc-900 border border-zinc-800 rounded-lg px-5 py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-xs text-zinc-400">{entry.serviceAccount}</span>
                      {entry.keyId !== "—" && (
                        <span className="text-[10px] text-zinc-500 font-mono">key:{entry.keyId}</span>
                      )}
                    </div>
                  ...
                  <div className="text-xs text-zinc-500 shrink-0">{entry.timestamp.slice(0, 16).replace("T", " ")}</div>
