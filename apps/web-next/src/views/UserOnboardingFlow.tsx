    ...
    <div className="min-h-screen bg-zinc-950 text-white p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Design, monitor, and optimize user activation journeys</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-400 mb-1">Users in Active Flows</div>
          <div className="text-2xl font-bold text-white">{totalActive.toLocaleString()}</div>
          <div className="text-xs text-zinc-500 mt-1">{FLOWS.filter(f => f.status === "active").length} active flows</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-400 mb-1">Avg Completion Rate</div>
          <div className="text-2xl font-bold text-emerald-400">{avgCompletion}%</div>
          <div className="text-xs text-zinc-500 mt-1">across active flows</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-400 mb-1">Users This Week</div>
          <div className="text-2xl font-bold text-indigo-400">261</div>
          <div className="text-xs text-zinc-500 mt-1">↑ 12% vs last week</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-400 mb-1">At-Risk Users</div>
          <div className="text-2xl font-bold text-amber-400">{USER_PROGRESS.filter(u => u.healthScore < 60).length}</div>
          <div className="text-xs text-zinc-500 mt-1">health score below 60</div>
        </div>
      </div>
            ...
            <div key={f.id} className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-zinc-500 mt-1">Target: <span className="font-mono text-zinc-400">{f.targetSegment}</span></div>
                  </div>
                  <div className="flex items-center gap-6 shrink-0 text-sm">
                    <div className="text-center">
                      <div className="text-xs text-zinc-500 mb-0.5">Users</div>
                      <div className="text-zinc-300">{f.totalUsers.toLocaleString()}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-zinc-500 mb-0.5">Completion</div>
                      <div className={f.completionRate >= 70 ? "text-emerald-400" : f.completionRate >= 50 ? "text-amber-400" : "text-rose-400"}>
                        {f.completionRate}%
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-zinc-500 mb-0.5">Avg Days</div>
                      <div className="text-zinc-300">{f.avgCompletionDays}</div>
                    </div>
                    <span className="text-zinc-600">{selectedFlow === f.id ? "▲" : "▼"}</span>
                  </div>
                </div>
              ...
              {selectedFlow === f.id && (
                <div className="border-t border-zinc-800 p-4 bg-zinc-950">
                  <h3 className="text-sm font-medium text-zinc-300 mb-3">Step Funnel</h3>
                  <div className="space-y-2">
                    {f.steps.map((step, idx) => (
                      <div key={step.id} className="flex items-center gap-3">
                        <span className="text-zinc-600 text-xs w-4 text-right">{idx + 1}</span>
      ...
      {tab === "users" && (
        <div className="space-y-4">
          <div className="flex gap-2">
            <select value={flowFilter} onChange={e => setFlowFilter(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded-md px-3 py-1.5 text-sm text-white">
              <option value="all">All Flows</option>
              {FLOWS.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
            </select>
            <span className="text-sm text-zinc-400 self-center">{filteredUsers.length} users</span>
          </div>
          ...
          <div className="space-y-3">
            {filteredUsers.map(u => {
              ...
              return (
                <div key={u.userId} className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
                  <button
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-zinc-500 mt-0.5">{userFlow?.name} · Day {u.daysInFlow}</div>
                      </div>
                      {/* Step indicators */}
                  ...
                  {selectedUser === u.userId && (
                    <div className="border-t border-zinc-800 p-4 bg-zinc-950">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="text-xs text-zinc-400 uppercase mb-2">Journey Progress</div>
                          <div className="space-y-2">
                            {userFlow?.steps.map((step, idx) => (
                              <div key={step.id} className="flex items-center gap-2 text-sm">
                                ...
                                <span className={cn(
                                  u.steps[idx] === "completed" ? "text-zinc-400 line-through" :
                                  u.steps[idx] === "current" ? "text-white font-medium" :
                                  u.steps[idx] === "skipped" ? "text-zinc-600" : "text-zinc-500"
                                )}>{step.name}</span>
                              </div>
                            ))}
                          </div>
                        ...
                        <div>
                          <div className="text-xs text-zinc-400 uppercase mb-2">Activity</div>
                          <div className="space-y-1.5 text-sm">
                            <div className="flex justify-between">
                              <span className="text-zinc-500">Started</span>
                              <span className="text-zinc-300">{u.startedAt}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-zinc-500">Last active</span>
                              <span className="text-zinc-300">{u.lastActiveAt}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-zinc-500">Days in flow</span>
                              <span className="text-zinc-300">{u.daysInFlow}</span>
                            </div>
                          </div>
                          {u.healthScore < 60 && (
      ...
      {tab === "cohorts" && (
        <div className="space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
            <h3 className="text-sm font-medium text-zinc-300 mb-4">Weekly Onboarding Cohorts — Developer Quick Start</h3>
              ...
              {COHORTS.slice(0, 7).map(c => (
                <div key={c.week} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex flex-col justify-end gap-px" style={{ height: "120px" }}>
                    ...
                    <div className="w-full bg-zinc-700" style={{ height: `${((c.started - c.completed) / maxCohortStarted) * 120}px` }} />
                  </div>
                  <div className="text-xs text-zinc-400" style={{ fontSize: "10px" }}>{c.week}</div>
                  <div className="text-xs text-zinc-300">{c.rate > 0 ? `${c.rate}%` : "—"}</div>
                </div>
              ))}
            </div>
            <div className="flex gap-4 text-xs text-zinc-400">
              ...
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-zinc-700 inline-block" /> Incomplete</span>
            </div>
          </div>

          <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
            <div className="p-3 border-b border-zinc-800">
              <h3 className="text-sm font-medium text-zinc-300">Cohort Details</h3>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="text-left p-3 text-zinc-400 font-medium">Week</th>
                  <th className="text-right p-3 text-zinc-400 font-medium">Started</th>
                  <th className="text-right p-3 text-zinc-400 font-medium">Completed</th>
                  <th className="text-right p-3 text-zinc-400 font-medium">Rate</th>
                  <th className="text-right p-3 text-zinc-400 font-medium">Trend</th>
                </tr>
              </thead>
              <tbody>
                {COHORTS.map(c => (
                  <tr key={c.week} className="border-b border-zinc-800/50 hover:bg-zinc-800/20">
                    <td className="p-3 text-zinc-300">{c.week}</td>
                    <td className="p-3 text-right text-zinc-300">{c.started}</td>
                    <td className="p-3 text-right text-zinc-300">{c.completed || "—"}</td>
                    ...
                    <td className="p-3 text-right">
                      <span className={c.trend === "up" ? "text-emerald-400" : c.trend === "down" ? "text-rose-400" : "text-zinc-500"}>
      ...
      {tab === "editor" && (
        <div className="flex gap-4">
          <div className="w-56 space-y-1 shrink-0">
            <div className="text-xs font-medium text-zinc-500 uppercase tracking-wider px-2 mb-2">Select Flow</div>
            {FLOWS.map(f => (
              <button
                key={f.id}
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {flow.steps.map((step, idx) => (
                    <div key={step.id} className="flex items-start gap-3 bg-zinc-800 border border-zinc-700 rounded-md p-3">
                      <span className="text-zinc-500 text-sm w-5 mt-0.5 text-right shrink-0">{idx + 1}</span>
                      <span className="text-lg shrink-0">{stepTypeIcon[step.type]}</span>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-medium">{step.name}</span>
                          <span className="text-xs px-1.5 py-0.5 bg-zinc-700 text-zinc-400 rounded">{step.type}</span>
                        </div>
                        <p className="text-xs text-zinc-500">{step.description}</p>
                      </div>
                      <div className="text-xs text-zinc-500 shrink-0 text-right">
                        <div>{step.completionRate}% complete</div>
                        <div className="text-zinc-600">{step.avgCompletionTimeH}h avg</div>
                      </div>
                      <div className="flex gap-1 shrink-0">
                        <button className="p-1 hover:bg-zinc-700 rounded text-zinc-500 hover:text-zinc-300">✏️</button>
                        <button className="p-1 hover:bg-zinc-700 rounded text-zinc-500 hover:text-zinc-300">🗑️</button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex gap-2 mt-4 pt-4 border-t border-zinc-800">
                  ...
                  <button className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-md text-sm text-zinc-300 transition-colors">Preview</button>
                </div>
              </div>
            ) : (
