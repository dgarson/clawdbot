...
type ObsidianSyncMode = "direct" | "rest-api" | "node-bridge";
...
function ObsidianIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M11.98 1.5c-1.1 0-2.07.62-2.53 1.62L2.2 17.53c-.25.5-.34 1.08-.24 1.63l1.1 3.75c.17.57.7.96 1.29.96h15.29c.59 0 1.12-.39 1.29-.96l1.1-3.75c.1-.55.01-1.13-.24-1.63L14.51 3.12c-.46-1-1.43-1.62-2.53-1.62zm-.31 3.03 6.74 13.7h-2.85l-3.89-7.86-3.9 7.86H4.92l6.75-13.7z" />
    </svg>
  );
}
...
const CONNECTION_DEFINITIONS: Connection[] = [
  ...
  },
  {
    id: "obsidian",
    name: "Obsidian",
    icon: <ObsidianIcon className="h-6 w-6" />,
    description: "Link your Obsidian vault for local knowledge sync",
    authMethods: [
      {
        id: "obsidian-rest-api",
        label: "Local REST API",
        description: "Connect using the Local REST API plugin",
        type: "api_key",
        badge: "Recommended",
        fields: [
          {
            id: "apiKey",
            label: "API Key",
            placeholder: "obsidian_xxxxxxxxxxxxxx",
            type: "password",
            required: true,
          },
          {
            ...
            helpText: "Leave blank to use the default (http://localhost:27123).",
            required: false,
          },
        ],
      },
      {
        id: "obsidian-direct",
        label: "Direct Vault Path",
        description: "Read directly from a local vault folder",
        type: "api_key",
        fields: [
          {
            id: "vaultPath",
      ...
      {
        id: "obsidian-node-bridge",
        label: "Node Bridge",
        description: "Connect to a remote vault via a node bridge",
        type: "api_key",
        fields: [
          {
            id: "nodeId",
    ...
    syncOptions: [
      { id: "syncNotes", label: "Sync notes", description: "Keep notes synced with your vault.", defaultEnabled: true },
      { id: "watchChanges", label: "Watch for changes", description: "Sync updates as files change.", defaultEnabled: true },
      { id: "indexWikiLinks", label: "Index wiki links", description: "Build a link index for wiki-style links.", defaultEnabled: true },
      { id: "indexTags", label: "Index tags", description: "Track tags across your vault.", defaultEnabled: true },
      { id: "ingestToMemory", label: "Ingest to memory", description: "Send notes into memory pipelines.", defaultEnabled: false },
    ],
  },
  {
    id: "linear",
...
export function ConnectionsSectionWithOAuth({
  ...
  const { data: configSnapshot, refetch: refetchConfig } = useConfig();
  ...
  const obsidianConfig = React.useMemo<ObsidianConfig | null>(() => {
    const raw = configSnapshot?.config?.obsidian;
    if (!isPlainObject(raw)) {
      return null;
    }
    return raw as ObsidianConfig;
  }, [configSnapshot]);

  const obsidianConnected = Boolean(obsidianConfig?.enabled);
  const obsidianSyncMode: ObsidianSyncMode = obsidianConfig?.syncMode ?? "direct";
  const obsidianSyncModeLabel = {
    direct: "Direct Vault",
    "rest-api": "Local REST API",
    "node-bridge": "Node Bridge",
  }[obsidianSyncMode];
  const obsidianDetail =
    obsidianSyncMode === "rest-api"
      ? obsidianConfig?.restApi?.url ?? "https://localhost:27124"
      : obsidianSyncMode === "node-bridge"
        ? obsidianConfig?.nodeBridge?.remoteVaultPath
  ...
  const handleRefreshStatuses = async () => {
    setRefreshing(true);
    await Promise.all([fetchAllStatuses(OAUTH_ENABLED_PROVIDERS), refetchConfig()]);
    setRefreshing(false);
  };
  ...
  const handleDisconnect = async (connectionId: string) => {
    if (connectionId === "obsidian") {
      ...
      const currentObsidian = isPlainObject(obsidianConfig) ? obsidianConfig : {};
      await patchConfig.mutateAsync({
        ...
        note: "Disconnect Obsidian integration",
      });
      return;
    }
  ...
  const getConnectionStatus = (connectionId: string): ConnectionStatus => {
    if (connectionId === "obsidian") {
      return { connected: obsidianConnected };
    }
      ...
      <CardContent>
        ...
        <div className="space-y-4">
          {CONNECTION_DEFINITIONS.map((connection) => {
            ...
            const showObsidianDetails = connection.id === "obsidian" && isConnected;
      ...
      {activeConnection && (
        <ConnectionWizardWithScopes
          ...
          onConnect={async (payload) => {
            if (activeConnection.id === "obsidian") {
              if (!configSnapshot?.hash) {return;}
              const currentObsidian = isPlainObject(obsidianConfig) ? obsidianConfig : {};
              const currentWatcher = isPlainObject(currentObsidian.watcher)
                ? currentObsidian.watcher
                : {};
              const currentMemoryIngest = isPlainObject(currentObsidian.memoryIngest)
                ? currentObsidian.memoryIngest
                : {};
              const syncMode =
                payload.authMethodId === "obsidian-rest-api"
                  ? "rest-api"
                  : payload.authMethodId === "obsidian-node-bridge"
                    ? "node-bridge"
                    : "direct";
              const nextObsidian: ObsidianConfig = {
                ...currentObsidian,
                enabled: payload.options.syncNotes ?? true,
                syncMode,
                watcher: {
                  ...currentWatcher,
                  enabled: payload.options.watchChanges ?? true,
                },
                memoryIngest: {
                  ...currentMemoryIngest,
                  enabled: payload.options.ingestToMemory ?? false,
                  indexWikiLinks: payload.options.indexWikiLinks ?? true,
                  indexTags: payload.options.indexTags ?? true,
                },
              };

              if (syncMode === "rest-api") {
                nextObsidian.restApi = {
                  ...(isPlainObject(currentObsidian.restApi) ? currentObsidian.restApi : {}),
                  apiKey: payload.values.apiKey?.trim(),
                };
                if (payload.values.url?.trim()) {
                  nextObsidian.restApi.url = payload.values.url.trim();
                }
              }

              if (syncMode === "direct") {
                nextObsidian.vaultPath = payload.values.vaultPath?.trim();
              }

              if (syncMode === "node-bridge") {
                nextObsidian.nodeBridge = {
                  ...(isPlainObject(currentObsidian.nodeBridge) ? currentObsidian.nodeBridge : {}),
                  nodeId: payload.values.nodeId?.trim(),
                  remoteVaultPath: payload.values.remoteVaultPath?.trim(),
                };
              }

              await patchConfig.mutateAsync({
                baseHash: configSnapshot.hash,
                raw: JSON.stringify({ obsidian: nextObsidian }),
                note: "Update Obsidian integration",
              });
              return;
            }
