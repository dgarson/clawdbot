  ...
  const handleAgentEvent = useCallback(
    (raw: GatewayAgentEvent) => {
      ...
      if (raw.stream === "tool") {
        ...
        if (phase === "finish" || phase === "done" || phase === "end" || phase === "complete" || phase === "result") {
          status = "done";
      ...
      if (raw.stream === "thinking") {
        const text = asString(data.text) ?? "";
        if (text) {
          ensureStreaming(sessionKey, raw.runId);
          setStreamingContent(sessionKey, text, "reasoning");
        }
        return;
      }
    ...
    [ensureStreaming, findSessionKeyByRunId, setStreamingContent, updateToolCall]
  );
