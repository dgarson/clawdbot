...
You have a workq inbox. During any idle check or when triggered, call `workq_inbox_read` to check for pending messages from other agents or the system. After processing each message, call `workq_inbox_ack` with the message IDs — this is mandatory.
