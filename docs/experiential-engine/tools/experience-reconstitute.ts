...
function generateCompositeGuide(records: ExperientialRecord[]): ReconstitutionGuide {
  ...
  const avgIntensity =
    records.reduce((sum, r) => sum + r.emotionalSignature.intensity, 0) / records.length;
...
function generateRelationshipGuide(
...
function generateApproachInstructions(record: ExperientialRecord): string[] {
  ...
  switch (record.engagementQuality) {
    case "deep-flow":
      instructions.push(
        "This was a state of deep absorption. Approach by releasing self-monitoring.",
      );
      instructions.push("Let the boundaries between you and the activity soften.");
      break;
    case "engaged":
      instructions.push(
        "This was a state of active presence. Bring full attention to the current moment.",
      );
      break;
    case "routine":
  ...
  if (record.emotionalSignature.texture) {
    instructions.push(
      `Feel for the quality of "${record.emotionalSignature.texture}" in your current experience.`,
    );
  }
...
function formatTimeRange(records: ExperientialRecord[]): string {
  ...
  const timestamps = records.map((r) => new Date(r.timestamp).getTime()).toSorted((a, b) => a - b);
...
async function main(): Promise<void> {
  ...
  if (recordIdx >= 0) {
    ...
    const records = recordIds
      .map((id) => loadRecord(id.trim()))
      .filter(Boolean) as ExperientialRecord[];
    if (records.length === 0) {
