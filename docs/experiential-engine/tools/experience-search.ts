...
function keywordSearch(records: ExperientialRecord[], query: SearchQuery): SearchResult[] {
  ...
  for (const record of records) {
    ...
    // Time filtering
    if (query.after) {
      if (new Date(record.timestamp) < query.after) {
        continue;
      }
    }
    if (query.before) {
      if (new Date(record.timestamp) > query.before) {
        continue;
      }
    }
    ...
    if (query.intensity) {
      ...
      if (query.intensity.min !== undefined && intensity < query.intensity.min) {
        continue;
      }
      if (query.intensity.max !== undefined && intensity > query.intensity.max) {
        continue;
      }
      score += 5;
    ...
    if (query.valence) {
      ...
      if (valence !== undefined) {
        if (query.valence.min !== undefined && valence < query.valence.min) {
          continue;
        }
        if (query.valence.max !== undefined && valence > query.valence.max) {
          continue;
        }
        score += 5;
    ...
    if (query.tags && query.tags.length > 0) {
      ...
      const matches = query.tags.filter((t) => recordTags.has(t.toLowerCase()));
      if (matches.length > 0) {
...
function generateRelevanceExplanation(
  ...
  const contextMatches = matchReasons.filter(
    (r) => r.startsWith("Relationship") || r.startsWith("Topic"),
  );
...
function getEmbeddableText(record: ExperientialRecord): string {
  return [
    ...
    record.emotionalSignature.secondary
      ? `Secondary emotions: ${record.emotionalSignature.secondary.join(", ")}`
      : "",
...
async function executeSearch(query: SearchQuery): Promise<SearchResponse> {
  ...
  if (query.text && embeddingsAvailable && embeddingProvider) {
    ...
    results = Array.from(scoreMap.values()).toSorted((a, b) => b.score - a.score);
    searchMethod = "hybrid";
  } else {
    // Keyword-only search
...
function printResults(response: SearchResponse): void {
  ...
  response.results.forEach((result, i) => {
    ...
    if (record.context.relationship) {
      console.log(`    Relationship: ${record.context.relationship}`);
    }
...
function printResultsCompact(response: SearchResponse): void {
  response.results.forEach((result) => {
    ...
    console.log(
      `${r.id.slice(0, 8)} | ${r.emotionalSignature.primary[0]} | ${r.context.topic || r.context.relationship || "untitled"} | ${result.score}`,
    );
  });
}
...
// Exports
export { SearchQuery, SearchResult, SearchResponse, executeSearch, getEmbeddableText };
