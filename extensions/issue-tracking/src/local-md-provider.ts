...
function isMetadataEqual(
  ...
  return leftKeys.every(
    (key) => Object.prototype.hasOwnProperty.call(right, key) && right[key] === left[key],
  );
}

function isSameReference(left: TicketReference, right: TicketReference): boolean {
  return (
    left.id === right.id &&
    left.kind === right.kind &&
    left.title === right.title &&
    left.uri === right.uri &&
    isMetadataEqual(left.metadata, right.metadata)
  );
}

function isSameRelationship(left: TicketRelationship, right: TicketRelationship): boolean {
  return left.kind === right.kind && left.ticketId === right.ticketId;
}

function mergeUniqueReferences(
  current: TicketReference[],
  appended: TicketReference[] | undefined,
): TicketReference[] {
  const merged: TicketReference[] = [...current];
  for (const reference of appended ?? []) {
    if (!merged.some((candidate) => isSameReference(candidate, reference))) {
...
function mergeUniqueRelationships(
  current: TicketRelationship[],
  appended: TicketRelationship[] | undefined,
): TicketRelationship[] {
  const merged: TicketRelationship[] = [...current];
  for (const relationship of appended ?? []) {
    if (!merged.some((candidate) => isSameRelationship(candidate, relationship))) {
...
function mergeUpdate(ticket: IssueTicket, input: IssueTicketUpdateInput): IssueTicket {
  ...
  return {
    ...
    references: mergeUniqueReferences(ticket.references, input.appendReferences),
    relationships: mergeUniqueRelationships(ticket.relationships, input.appendRelationships),
    updatedAt: now,
  };
}
