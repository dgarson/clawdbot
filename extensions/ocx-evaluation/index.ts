...
export default function register(api: OpenClawPluginApi) {
  ...
  api.on("after_tool_call", (event, ctx) => {
    // Attach tool events to the current run.
    // Without a runId on tool hooks, we key by sessionKey.
    const sessionKey = ctx.sessionKey ?? "";
    const runEntry = findRunBySessionKey(sessionKey);
    if (!runEntry) return;

    const toolEvents = runEntry.toolEvents ?? [];
    toolEvents.push({
      eventId: `tool-${randomUUID()}`,
  ...
  api.on("agent_end", (event, ctx) => {
    // Find the pending run for this session and mark as complete
    const sessionKey = ctx.sessionKey ?? "";
    const runEntry = findRunBySessionKey(sessionKey);
    if (!runEntry || !runEntry.runId) return;
    ...
    pendingRuns.delete(runEntry.runId);
  });

  /** Find a pending run by sessionKey (linear scan; small map). */
  function findRunBySessionKey(sessionKey: string): Partial<PendingRun> | undefined {
    for (const entry of pendingRuns.values()) {
  ...
  const deps: EvaluatorDeps = {
    ...
    emitEvent(event) {
      ...
      // We use a synthetic runId scoped to the evaluation plugin since these events
      // originate from the background scorer, not from a user-facing agent run.
      emitAgentEvent({
        runId: `eval-${event.data.runId ?? "global"}`,
        stream: event.family,
