...
export function registerCollector(
  ...
  api.on("message_received", (event, ctx) => {
    emit(storage, config, "message", "message.received", randomUUID(), {
  ...
  api.on("message_sent", (event, ctx) => {
    emit(storage, config, "message", "message.sent", randomUUID(), {
      channel: ctx.channelId,
      to: event.to,
      contentLength: event.content.length,
  ...
  api.on("gateway_start", (event) => {
    emit(storage, config, "system", "system.gateway.start", randomUUID(), {
      port: event.port,
    });
  });

  api.on("gateway_stop", (event) => {
    emit(storage, config, "system", "system.gateway.stop", randomUUID(), {
