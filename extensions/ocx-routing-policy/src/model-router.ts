...
export function buildMatchContext(
  ...
  const budgetRemainingRaw = event.agentMetadata?.budgetRemaining;
  const budgetRemaining =
    typeof budgetRemainingRaw === "number" && Number.isFinite(budgetRemainingRaw)
      ? budgetRemainingRaw
      : undefined;
  return {
    agentId: ctx.agentId,
    ...
    classification,
    budgetRemaining,
    toolCount: Array.isArray(event.toolsAvailable) ? event.toolsAvailable.length : undefined,
    sessionDepth: typeof event.spawnDepth === "number" ? event.spawnDepth : undefined,
  };
}
