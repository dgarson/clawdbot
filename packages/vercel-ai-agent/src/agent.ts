...
export class ConversationalAgent {
  ...
  async run(input: AgentRunInput): Promise<AgentResponse> {
    ...
    const callbacks = mergeCallbacks(this.config.defaultCallbacks, input.callbacks);
    ...
    try {
      ...
      while (shouldContinue && stepNumber < (executionConfig.maxSteps || 10)) {
        ...
        const step: StepResult = {
          stepNumber,
          stepType:
            stepNumber === 0 ? "initial" : result.toolCalls.length > 0 ? "tool-result" : "continue",
          text: result.text,
  ...
  async runStream(input: AgentRunInput): Promise<AgentStreamResponse> {
    const executionConfig = mergeExecutionConfig(this.config.defaultExecutionConfig, {
      ...input.executionConfig,
      stream: true,
    });
    const callbacks = mergeCallbacks(this.config.defaultCallbacks, input.callbacks);
  ...
  async chat(
    ...
    const shouldStream =
      options?.executionConfig?.stream ?? this.config.defaultExecutionConfig?.stream ?? false;
