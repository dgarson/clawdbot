...
function loadEnv() {
  ...
  const candidates = [
    ...
    resolve(__dirname, '../secrets/twilio.env'),
  ];
...
async function twilioRequest(method, path, body = null) {
  const auth = Buffer.from(`${CONFIG.twilio.apiKeySid}:${CONFIG.twilio.apiKeySecret}`).toString('base64');
...
async function updatePhoneNumberWebhook(voiceUrl) {
  ...
  const result = await twilioRequest('POST', `/IncomingPhoneNumbers/${CONFIG.twilio.phoneNumberSid}.json`, {
    VoiceUrl: voiceUrl, VoiceMethod: 'POST',
  });
  if (result.sid) { console.log(`✅ Phone ${result.phone_number} → tunnel`); return true; }
...
function linearToMulaw(sample) {
  ...
  let exponent = 7;
  let expMask = 0x4000;
  while ((sample & expMask) === 0 && exponent > 0) { exponent--; expMask >>= 1; }
...
async function textToSpeechStream(text, streamSid, twilioWs, cancelToken = { cancelled: false }) {
  ...
  for (let offset = 0; offset < mulaw.length; offset += CHUNK) {
    ...
    if (twilioWs?.readyState === WebSocket.OPEN && streamSid) {
      twilioWs.send(JSON.stringify({
        event: 'media', streamSid, media: { payload: Buffer.from(chunk).toString('base64') },
      }));
      chunkCount++;
    }
  ...
  if (!cancelToken.cancelled) {
    ...
    if (twilioWs?.readyState === WebSocket.OPEN && streamSid) {
      twilioWs.send(JSON.stringify({ event: 'mark', streamSid, mark: { name: 'tts-done-' + Date.now() } }));
    }
  }
}
...
class OpenAIRealtimeSTT {
  ...
  connect() {
    ...
    this.ws.on('close', () => { this.isConnected = false; console.log('  🎤 OpenAI STT disconnected'); });
  }
...
class GatewayBrain {
  constructor() {
    this.ws = null;
    this.isConnected = false;
    this.pendingRequests = new Map();  // id → { resolve, reject, expectFinal, timer }
    ...
    this.ws.on('message', (data) => {
      try {
        ...
        if (msg.type === 'res') {
          ...
          // If waiting for final response and this is just an ack, keep waiting
  ...
  /**
   * Send user text to Claude via the gateway's `agent` method.
   * Waits for the final response (status:"ok") before resolving.
  ...
  async think(userText, timeoutMs = 30000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pendingRequests.set(reqId, {
        resolve: (payload) => {
          // payload: { runId, status:"ok", result:{ payloads:[{text}], meta } }
          const text = payload?.result?.payloads?.[0]?.text?.trim()
            ?? payload?.summary
            ?? '';
          resolve(text || "I'm not sure how to respond to that.");
        },
        reject,
        expectFinal: true,
        timer,
      ...
      console.log(`  🧠 → Gateway: "${userText.slice(0, 70)}${userText.length > 70 ? '...' : ''}" [${reqId.slice(0, 8)}]`);
    });
  }
  ...
  cancelAll() {
    for (const pending of this.pendingRequests.values()) {
      clearTimeout(pending.timer);
      pending.reject(new Error('Cancelled (barge-in)'));
    }
    this.pendingRequests.clear();
  }
...
class VoiceSession {
  ...
  async start() {
    ...
    if (this.brain.isConnected) {
      try {
        greeting = await this.brain.think('[SYSTEM: Call just connected. Greet the caller warmly and briefly.]', 10000);
      } catch {
  ...
  handleTranscript(transcript, isFinal) {
    if (isFinal) {
      ...
      if (this.isSpeaking && transcript.trim().length >= CONFIG.bargeIn.minChars) {
        this.triggerBargeIn();
      }
    }
  }
  ...
  handleUtteranceEnd() {
    ...
    if (isGoodbye) {
      ...
      const textToSend = wasInterrupted
        ? `[User interrupted you to say:] ${fullText}`
        : fullText;
  ...
  triggerBargeIn() {
    const now = Date.now();
    if (now - this.lastBargeInTime < CONFIG.bargeIn.cooldownMs) return;
    ...
    if (this.currentTTSCancel) this.currentTTSCancel.cancelled = true;

    this.currentGenerationId++;
    this.brain.cancelAll();
  ...
  async respond(userText) {
    ...
    try {
      const t0 = Date.now();
      const responseText = await this.brain.think(userText);

      if (this.currentGenerationId !== myGeneration) {
        console.log(`  ⏩ Response discarded (barge-in during thinking) [gen ${myGeneration}]`);
        return;
      }

      console.log(`  🧠 ← Gateway (${Date.now() - t0}ms): "${responseText.slice(0, 80)}${responseText.length > 80 ? '...' : ''}"`);

      await this.speak(responseText);
    } catch (err) {
      if (this.currentGenerationId !== myGeneration) return;
      if (err.message.includes('Cancelled')) return;

      console.error('  ❌ respond error:', err.message);
      if (this.currentGenerationId === myGeneration) {
        await this.speak("I'm sorry, I'm having a little trouble thinking right now. Could you say that again?");
      }
    } finally {
      if (this.currentGenerationId === myGeneration) {
        this.isProcessing = false;
  ...
  async respondAndClose(userText) {
    const myGeneration = ++this.currentGenerationId;
    this.isProcessing = true;
    try {
      const responseText = await this.brain.think(userText);
      if (this.currentGenerationId !== myGeneration) return;
      console.log(`  👋 Goodbye: "${responseText}"`);
      await this.speak(responseText);
      this.brain.notify('call-end');
      setTimeout(() => { console.log('📞 Call ended (goodbye)'); this.close(); }, 5000);
    } catch (err) {
      if (this.currentGenerationId !== myGeneration) return;
  ...
  sendAudioToSTT(b64) { this.stt?.sendAudio(b64); }
...
wss.on('connection', (ws) => {
  ...
  ws.on('close', () => { console.log('📞 Twilio disconnected'); session?.close(); });
...
function startTunnel() {
  return new Promise((resolve, reject) => {
    ...
    const cf = spawn('cloudflared', ['tunnel', '--url', `http://localhost:${CONFIG.port}`], { stdio: ['ignore', 'pipe', 'pipe'] });
    let found = false;
    function parse(data) {
      const match = data.toString().match(/https:\/\/[a-z0-9-]+\.trycloudflare\.com/);
      if (match && !found) { found = true; tunnelUrl = match[0]; console.log(`🔒 Tunnel: ${tunnelUrl}`); resolve(tunnelUrl); }
    }
    cf.stdout.on('data', parse);
    cf.stderr.on('data', parse);
    cf.on('error', reject);
    cf.on('close', (code) => { if (!found) reject(new Error(`Tunnel exited (${code})`)); });
    setTimeout(() => { if (!found) reject(new Error('Tunnel timeout')); }, 15000);
