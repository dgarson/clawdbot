    ...
    return new Promise((resolve, reject) => {
      ...
      this.pendingRequests.set(reqId, {
        resolve: (payload) => {
          ...
          const text = payload?.result?.payloads?.[0]?.text?.trim() ?? payload?.summary ?? "";
      ...
      console.log(
        `  🧠 → Gateway: "${userText.slice(0, 70)}${userText.length > 70 ? "..." : ""}" [${reqId.slice(0, 8)}]`,
      );
    });
  }
...
class VoiceSession {
  ...
  async respond(userText) {
    ...
    try {
      ...
      console.log(
        `  🧠 ← Gateway (${Date.now() - t0}ms): "${responseText.slice(0, 80)}${responseText.length > 80 ? "..." : ""}"`,
      );
