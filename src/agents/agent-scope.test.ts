...
describe("resolveAgentConfig", () => {
  ...
  it("resolves agent model fallbacks override", () => {
    const cfg: OpenClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    expect(resolveAgentModelFallbacksOverride(cfg, "support")).toEqual(["openai/gpt-5.2"]);
    // Agent without explicit fallbacks returns undefined (inherits defaults)
    expect(resolveAgentModelFallbacksOverride(cfg, "main")).toBeUndefined();
  });
