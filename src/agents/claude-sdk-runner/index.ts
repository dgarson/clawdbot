...
import { getSdkSessionId, setSdkSessionId } from "./session-store.js";
...
export async function runClaudeSdkAgent(
  ...
  try {
    ...
    // Look up SDK session ID from previous turn (for resumption)
    const sdkSessionIdToResume = getSdkSessionId({
    ...
    // Store SDK session ID for future resumption
    if (result.sessionId) {
      setSdkSessionId({
        sessionId: params.sessionId,
    ...
    log.error(`CCSDK ERROR: ${err instanceof Error ? err.message : String(err)}`);
    log.error(`CCSDK ERROR stack: ${err instanceof Error ? err.stack : "no stack"}`);
    throw err;
  }
}
