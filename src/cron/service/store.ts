...
export function migrateJob(raw: Record<string, unknown>, _log: Logger): boolean {
  ...
  // Infer payload from top-level message/text if payload is missing
  ...
  // Strip top-level fields that have been copied into payload
...
export async function ensureLoaded(
  ...
  for (const raw of jobs) {
    const rawState = (
      raw.state && typeof raw.state === "object" && !Array.isArray(raw.state) ? raw.state : {}
    ) as Record<string, unknown>;
    if (raw.state !== rawState) {
      raw.state = rawState;
      mutated = true;
    }
    try {
