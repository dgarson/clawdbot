...
/**
 * Default scenarios loaded from the evaluation harness
 * These are registered at build time for CI compatibility
 */
const DEFAULT_SCENARIOS: CataloguedEvaluationCase[] = [
  // Sample smoke case (from existing sample-case.ts)
  {
    ...
    metadata: {
      ...
      assertions: ["harness wiring works"],
    },
  },
  // HITL scenarios
  hitlEscalationCase,
...
/**
 * Creates a scenario catalog from default registered scenarios
 * This is the primary entry point for CI to load all scenarios
 */
...
/**
 * Load scenarios by difficulty for tiered CI execution
 *
...
/**
 * Load scenarios by suite for suite-specific CI runs
 *
...
export async function loadScenariosFromFixture(
  ...
  for (const item of parsed) {
    if (!validateScenarioMetadata(item)) {
      console.warn(`Skipping invalid scenario in fixture: ${item.id ?? "unknown"}`);
      continue;
    }
    scenarios.push(item);
  }
