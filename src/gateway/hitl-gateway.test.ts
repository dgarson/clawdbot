...
describe("hitl-gateway", () => {
  ...
  describe("expireRequest", () => {
    ...
    it("escalates instead of expiring when escalation threshold is reached", () => {
      ...
      // Advance time past expiry (120_000ms) and past the afterTimeoutMs threshold (30_000ms).
      // afterTimeoutMs is measured from expiresAtMs, so we need timeoutMs + afterTimeoutMs + 1.
      nowMs += 120_000 + 31_000;
  ...
  // ─── approvalManager wiring ──────────────────────────────────────────────────

  describe("approvalManager wiring", () => {
    it("registers in-memory approval when gate fires so awaitDecision resolves on approve", async () => {
      const gateResult = gateway.checkAndGate({
        tool: "exec.run",
        requesterSession: "user-1",
        requesterRole: "user",
      ...
      // awaitDecision should be available immediately after checkAndGate
      const promise = approvalManager.awaitDecision(gateResult.requestId);
      expect(promise).not.toBeNull();

      // Record decision — should resolve the in-memory promise
      gateway.recordDecision({
        requestId: gateResult.requestId,
        actorSession: "operator-1",
        actorRole: "operator",
    ...
    it("resolves in-memory promise with deny on denial decision", async () => {
      const gateResult = gateway.checkAndGate({
        tool: "exec.run",
        requesterSession: "user-1",
        requesterRole: "user",
    ...
    it("no approval registered when gate is not triggered", () => {
      gateway.checkAndGate({
        tool: "unknown.tool",
        requesterSession: "user-1",
        requesterRole: "user",
      });
      // No requestId returned, and no spurious registrations
      expect(approvalManager.awaitDecision("nonexistent-id")).toBeNull();
    });
  });

  // ─── recordDecision security ─────────────────────────────────────────────────

  describe("recordDecision security", () => {
    it("rejects decision on expired request (past expiresAtMs)", () => {
      const gateResult = gateway.checkAndGate({
        tool: "exec.run",
        requesterSession: "user-1",
        requesterRole: "user",
      ...
      nowMs += 60_001; // policy-exec has timeoutMs: 60_000
    ...
    it("rejects decision when policy not found in gateway config", () => {
      // Create a gateway with no policies registered but use a requestId from
      // a store whose request references an unknown policyId.
      const gateResult = gateway.checkAndGate({
        tool: "exec.run",
        requesterSession: "user-1",
        requesterRole: "user",
      ...
      // Create a second gateway that has NO policies configured (simulates restart / misconfiguration)
      const emptyGateway = createHitlGateway({
        policyEngine,
        requestStore: store,
        approvalManager,
      ...
      const result = emptyGateway.recordDecision({
        requestId: gateResult.requestId,
        actorSession: "operator-1",
        actorRole: "operator",
        decision: "approve",
  ...
  // ─── expireRequest escalation semantics ─────────────────────────────────────

  describe("expireRequest escalation semantics", () => {
    it("does not escalate before request has expired (afterTimeoutMs is measured from expiresAtMs)", () => {
      const gateResult = gateway.checkAndGate({
        tool: "system.run", // policy-strict: timeoutMs=120_000, afterTimeoutMs=30_000
        requesterSession: "user-1",
        requesterRole: "user",
      });
      expect(gateResult.gated).toBe(true);

      // Advance 50s — past afterTimeoutMs (30s) but NOT past timeoutMs (120s)
      nowMs += 50_000;

      // Should expire, NOT escalate, because we haven't passed expiresAtMs yet
      const result = gateway.expireRequest(gateResult.requestId, false);
      expect(result?.status).toBe("expired");
    });

    it("escalates when past expiresAtMs + afterTimeoutMs", () => {
      const gateResult = gateway.checkAndGate({
        tool: "system.run", // policy-strict: timeoutMs=120_000, afterTimeoutMs=30_000
        requesterSession: "user-1",
        requesterRole: "user",
      ...
      // Advance past expiry (120s) + afterTimeoutMs (30s)
      nowMs += 120_000 + 31_000;
