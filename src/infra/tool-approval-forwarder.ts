...
const log = createSubsystemLogger("gateway/tool-approvals");
...
export type ToolApprovalRequest = {
  ...
  request: {
    ...
    agentId?: string | null;
    requestHash: string;
    /** Legacy exec fields for backward-compatible text. */
    command?: string | null;
    cwd?: string | null;
    host?: string | null;
...
export type ToolApprovalForwarder = {
  handleRequested: (request: ToolApprovalRequest) => Promise<void>;
  handleResolved: (resolved: ToolApprovalResolved) => Promise<void>;
  stop: () => void;
};

export type ToolApprovalForwarderDeps = {
...
function buildRequestMessage(request: ToolApprovalRequest, nowMs: number): string {
  const isExec = request.request.toolName === "exec";
  const lines: string[] = [];

  if (isExec) {
    // Backward-compatible exec format
    lines.push("🔒 Exec approval required");
    ...
    // Generic tool format
    lines.push(`🔒 Tool approval required: ${request.request.toolName}`);
    lines.push(`ID: ${request.id}`);
    if (request.request.paramsSummary) {
      lines.push(`Params: ${request.request.paramsSummary}`);
    }
    if (request.request.riskClass) {
      lines.push(`Risk: ${request.request.riskClass}`);
    }
    if (request.request.sideEffects?.length) {
      lines.push(`Side effects: ${request.request.sideEffects.join(", ")}`);
    }
    if (request.request.agentId) {
      lines.push(`Agent: ${request.request.agentId}`);
    }
...
function decisionLabel(decision: ToolApprovalDecision): string {
...
function buildResolvedMessage(resolved: ToolApprovalResolved): string {
  const base = `✅ Tool approval ${decisionLabel(resolved.decision)}.`;
  const by = resolved.resolvedBy ? ` Resolved by ${resolved.resolvedBy}.` : "";
  return `${base}${by} ID: ${resolved.id}`;
}

function buildExpiredMessage(request: ToolApprovalRequest): string {
  return `⏱️ Tool approval expired. ID: ${request.id}`;
}
...
async function deliverToTargets(params: {
  ...
  const deliveries = params.targets.map(async (target) => {
    ...
    try {
      ...
      log.error(`tool approvals: failed to deliver to ${channel}:${target.to}: ${String(err)}`);
    }
  });
...
export function createToolApprovalForwarder(
  deps: ToolApprovalForwarderDeps = {},
  ...
  const handleResolved = async (resolved: ToolApprovalResolved) => {
