...
export function redactSensitiveText(text: string, options?: RedactOptions): string {
  ...
  if (resolved.patterns.length === 0) return text;
...
function redactSensitiveValueInner(
  ...
  if (prim !== value) return prim;
  if (!value || typeof value !== "object") return value;
  if (seen.has(value)) return "[Circular]";
  if (depth >= DEFAULT_REDACT_OBJECT_DEPTH) return "[MaxDepth]";
  seen.add(value);
  ...
  if (value instanceof URL) return redactText(value.toString(), patterns);
...
export function redactSensitiveValue(value: unknown, options?: RedactOptions): unknown {
  ...
  if (resolved.patterns.length === 0) return value;
...
export function redactSensitiveArgs(args: unknown[], options?: RedactOptions): unknown[] {
  if (!Array.isArray(args) || args.length === 0) return args;
  const resolved = resolveRedactOptions(options);
  if (resolved.mode === "off") return args;
  if (resolved.patterns.length === 0) return args;
  const seen = new WeakSet<object>();
