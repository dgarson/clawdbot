...
export async function resolveSlackThreadStarter(params: {
  ...
  try {
    ...
    const starter: SlackThreadStarter = {
      // For image-only root messages, synthesize placeholder text matching the
      // same format used by resolveSlackThreadHistory() for file-only replies.
      text: text || `[attached: ${message.files?.map((f) => f.name ?? "file").join(", ")}]`,
      userId: message.user,
      ts: message.ts,
      files: message.files,
    };
